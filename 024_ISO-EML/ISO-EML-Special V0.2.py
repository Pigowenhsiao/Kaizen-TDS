...
class IniSettings:
    ...
    def __init__(self):
        ...
        self.tool_name_map = {}
        # Pivot-specific (New)
        self.pivot_index_column = None
        self.pivot_labels_column = None
...
def _extract_settings_from_config(config):
    ...
    # Excel Pivot (New)
    if config.has_section('Excel_Pivot'):
        s.pivot_index_column = config.get('Excel_Pivot', 'pivot_index_column', fallback=None)
        s.pivot_labels_column = config.get('Excel_Pivot', 'pivot_labels_column', fallback=None)
        s.pivot_value_columns = [x.strip() for x in config.get('Excel_Pivot', 'pivot_value_columns').split(',')]
...
def process_excel_file(filepath_str, settings, log_file, csv_filepath):
    ...
    try:
        ...
        df_raw = pd.read_excel(filepath, header=None, sheet_name=settings.sheet_name, usecols=settings.data_columns, skiprows=start_row)
        Log.Log_Info(log_file, f"Step 1: Successfully read main sheet '{settings.sheet_name}', {df_raw.shape[0]} rows loaded.")
        
        # --- NEW LOGIC: DATA PIVOTING ---
        if settings.pivot_index_column and settings.pivot_labels_column and settings.pivot_value_columns:
            Log.Log_Info(log_file, "Pivoting logic detected. Starting data transformation...")
            
            # Create a column index map for easier access
            col_map = {column_index_from_string(v['col']): k for k, v in settings.field_map.items() if v['col'] != '-1'}
            df_raw.columns = [col_map.get(i, f'unused_{get_column_letter(i)}') for i in range(1, len(df_raw.columns) + 1)]
            
            # Identify pivot and non-pivot columns
            pivot_values = settings.pivot_value_columns
            pivot_index_col = next((k for k, v in settings.field_map.items() if v['col'] == settings.pivot_index_column), None)
            pivot_labels_col = next((k for k, v in settings.field_map.items() if v['col'] == settings.pivot_labels_column), None)
            
            non_pivot_cols = [col for col in df_raw.columns if col not in pivot_values and col not in [pivot_labels_col]]
            
            # Drop rows with no pivot label (A/B/C/D) to ensure clean pivot
            df_raw.dropna(subset=[pivot_labels_col], inplace=True)
            
            # Perform the pivot
            df_pivot = df_raw.pivot_table(
                index=non_pivot_cols,
                columns=pivot_labels_col,
                values=pivot_values,
                aggfunc='first'
            )
            ...
            Log.Log_Info(log_file, f"Data pivoting completed. New DataFrame has {df.shape[0]} rows.")
        else:
            Log.Log_Info(log_file, "No pivoting logic detected. Processing in standard mode.")
            ini_keys_by_col_index = {column_index_from_string(v['col']) - 1: k for k, v in settings.field_map.items() if v['col'] != '-1'}
            df_raw.columns = [ini_keys_by_col_index.get(i, f'unused_{i}') for i in range(df_raw.shape[1])]
            df = df_raw
        ...
        # Note: 'key_Serial_Number' is now 'key_Serial_Number_A' etc. after pivot. Need to adjust the column to check.
        # Assuming the pivot index column is what we need to check for non-nulls.
        if settings.pivot_index_column:
            df.dropna(subset=[pivot_index_col], inplace=True)
        else:
            df.dropna(subset=['key_Serial_Number'], inplace=True)
        Log.Log_Info(log_file, f"Step 2: Initial filtering (date, lot number/serial number) complete. {df.shape[0]} rows remaining.")
    except Exception as e:
    ...
    try:
        ...
        if conn is None: 
            ...
            return
        def get_db_info(lot_num): return pd.Series(SQL.selectSQL(cursor, str(lot_num)))
        df[['key_Part_Number', 'key_LotNumber_9']] = df['key_LotNumber'].apply(get_db_info)
    ...
    # Original dynamic renaming based on field_map
    for key, val in settings.field_map.items():
        if key not in ['key_LotNumber', 'key_Start_Date_Time', 'key_Part_Number', 'key_Serial_Number', 'key_TestEquipment_Nano', 'key_Tool_name']:
    ...
    special_renames = {
        ...
        'key_LotNumber': 'LotNumber',
        'key_Start_Date_Time': 'Start_Date_Time', 
        'key_TestEquipment_Nano': 'Nanospec_DeviceSerialNumber', 
        'key_Tool_name': 'DryEtch_DeviceSerialNumber',
        'key_LotNumber_9': 'LotNumber_9'
    }
    rename_map.update(special_renames)
    
    # Dynamically generate final column list, including pivoted columns
    pivoted_cols = [col for col in df.columns if any(p in col for p in settings.pivot_value_columns)]
    
    dynamic_column_order = ['LotNumber', 'Part_Number', 'Start_Date_Time', 'Operation', 'TestStation', 'Site', 'DryEtch_DeviceSerialNumber', 'LotNumber_9']
    dynamic_column_order.extend(pivoted_cols)
