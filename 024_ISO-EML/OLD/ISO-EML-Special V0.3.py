...
def _find_sql_func(name_keywords):
    """Find a callable in SQL module whose lowercased name contains all keywords."""
...
def _try_calls(fn, *pos_args, **kw_args):
    """Try calling a function with several signatures; return first non-None result."""
    sigs = [
        (),  # no args
        (pos_args,),  # df or df,site,op...
    ]
    # flatten signature attempts
    attempts = []
    if pos_args:
        attempts.append(pos_args)
    attempts.append(tuple())
    # also try kwargs if provided
    for args in attempts:
        try:
            ...
            # wrong signature; skip
            continue
        except Exception:
            # real error; re-raise to log
            raise
    return None

def enrich_via_sql(df: pd.DataFrame, settings, log_file):
    """
    Try SQL first for: STARTTIME_SORTED, SORTNUMBER, LotNumber_5, LotNumber_9.
    We auto-discover likely function names in SQL module and try multiple signatures.
    """
    try:
        # STARTTIME_SORTED
        fn, name = _find_sql_func(['start', 'time', 'sorted'])
        if not fn:
            fn, name = _find_sql_func(['starttime', 'sorted'])
        if fn:
            try:
                res = _try_calls(fn, df, settings.site, settings.operation)
                if res is not None:
                    df['STARTTIME_SORTED'] = res
                    Log.Log_Info(log_file, f"Filled STARTTIME_SORTED via SQL.{name}")
            except Exception as e:
                Log.Log_Error(log_file, f"SQL.{name} failed: {e}")

        # SORTNUMBER
        fn, name = _find_sql_func(['sort', 'number'])
        if fn:
            try:
                res = _try_calls(fn, df, settings.site, settings.operation)
                if res is not None:
                    df['SORTNUMBER'] = res
                    Log.Log_Info(log_file, f"Filled SORTNUMBER via SQL.{name}")
            except Exception as e:
                Log.Log_Error(log_file, f"SQL.{name} failed: {e}")

        # Lot numbers
        # Prefer a function that returns a DataFrame with LotNumber_5 and LotNumber_9
        fn, name = _find_sql_func(['lot', 'number'])
        if fn:
            try:
                lots = _try_calls(fn, df, settings.site, settings.operation)
                if isinstance(lots, pd.DataFrame):
                    if 'LotNumber_5' in lots.columns:
                        df['LotNumber_5'] = lots['LotNumber_5']
                    if 'LotNumber_9' in lots.columns:
                        df['LotNumber_9'] = lots['LotNumber_9']
                    Log.Log_Info(log_file, f"Filled lot numbers via SQL.{name}")
            except Exception as e:
                Log.Log_Error(log_file, f"SQL.{name} failed: {e}")
    except Exception as e:
        Log.Log_Error(log_file, f"SQL enrichment unexpected failure: {e}")
    finally:
        # Safe fallbacks
        ...
        if 'LotNumber_5' not in df or df['LotNumber_5'].isna().all():
            df['LotNumber_5'] = df['Serial_Number']
        if 'LotNumber_9' not in df or df['LotNumber_9'].isna().all():
            df['LotNumber_9'] = df['LotNumber_5']
        return df
...
def df_expand_to_sample(df_raw: pd.DataFrame, settings: IniSettings, log_file) -> pd.DataFrame:
    ...
    # SQL-first enrichment (with robust function discovery and safe fallbacks)
