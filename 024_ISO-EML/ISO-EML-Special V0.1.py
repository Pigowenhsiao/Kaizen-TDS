...
def df_expand_to_sample(df_raw: pd.DataFrame, settings: IniSettings, log_file) -> pd.DataFrame:
    ...
    pos_rows = df_raw[df_raw[c_Pos].astype(str).str.strip().isin(list('ABCD'))].copy()
    if pos_rows.empty:
    ...
    pos_rows['pos_i'] = pos_rows[c_Pos].astype(str).str.strip().map(pos_idx)
    ...
    def assemble(group):
        ...
        # Fill constants for later stages
    ...
    recs = []
    if c_SN is None or c_Start is None:
        raise RuntimeError("DataFields must include key_Serial_Number and key_Start_Date_Time.")
    for _, g in pos_rows.groupby([c_SN, c_Start], dropna=False):
        recs.append(assemble(g))
