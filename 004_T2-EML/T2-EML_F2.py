...
# ----- ログ書き込み先ファイルパス -----
Log_file = '../Log/' + Log_FolderName + '/004_T2-EML_F2.log'
...
def Open_Data_Sheet(Sheet, filepath, SheetName):
    ...
    Equipment1,Equipment2,Equipment3 = '1','1','2'
...
def Data_Extract(filepath, SheetList, old_check):
    ...
    for Sheet_Name in SheetList[::-1]:
        ...
        # ----- シートが処理対象シートかどうか確認 -----
        if '13B2-LD' not in str(Sheet['U3'].value) or Initial not in SerialNumber_list:
...
if __name__ == "__main__":
    ...
    shutil.copy("F2_FileName.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/004_T2-EML/13_ProgramUsedFile/')
