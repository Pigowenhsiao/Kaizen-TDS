...
Operation = 'N-electrode_EML_LiftOffPattern_Width'
...
Log_File = '../Log/' + Log_Folder_Name + '/035_N-electrode_EML_LiftOffPattern_Width.log'
Log.Log_Info(Log_File, 'Program Start')


########## 使用するシート名と開始行数を取り出すファイルを定義 ##########
Sheet_Text_Array = [
    ["HTL13B2・B4・B5・B6", "HTL13B2_etc_StartRow_PW.txt"],
    ["HTL13D1", "HTL13D1_StartRow_PW.txt"]
]


########## XML出力先ファイルパス ##########
Output_filepath = '//li.lumentuminc.net/data/SAG/TDS/Data/Files to Insert/XML/'
# Output_filepath = '../XML/035_N-electrode/'
...
def Main(Data_Sheet_Name, TextFile):
    ...
    Excel_File = shutil.copy(Excel_File_List[0][0], '../DataFile/035_N-electrode/')
    ...
    while Row_Number < row_end:
        ...
        # ----- 品種が 25G-EA のときに処理を継続 -----
        if Part_Class_Check(Part_Number) != '25G-EA':
        ...
        # 行数を取得し、[行数/10^6]を行う
        excel_row = Start_Number + df.index[Row_Number] + 1
    ...
    shutil.copy(TextFile, 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/035_N-electrode/13_ProgramUsedFile/')
