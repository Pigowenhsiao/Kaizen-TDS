...
#Output_filepath = 'C:/Users/hsi67063/Downloads/XML/'
...
Col_SEM_number = 2
Col_Serial_Number = 5
Col_ICP_Number = 7 # new add parameter
#From Here    shift 2 columns to right     2024-11-29 masaaki yoshida
Col_Width_1 = 14 #before=12 after=14    the same applies below
Col_Width_2 = 15
Col_Width_3 = 16
Col_Width_4 = 17
Col_Width_5 = 18
Col_WidthAve_Average = 19
Col_WidthAve_3sigma = 20
Col_DEKTAK_number = 21
Col_Thickness_1 = 22
Col_Thickness_2 = 23
Col_Thickness_3 = 24
Col_ThicknessAve_Average = 25
Col_ThicknessAve_3sigma = 26
#To Here
...
def Main():
    ...
    FileName = '*QC-K-008-R18*ﾒｻEB描画*.xls*' # 原來是'*QC-K-008-R11*ﾒｻEB描画*.xls*'
    Excel_File_List = []
    ...
    df = pd.read_excel(Excel_File, header=None, sheet_name=Data_Sheet_Name, usecols="B:AC", skiprows=Start_Number) #B:Z -> B:AB     2024-11-29 masaaki yoshida
    ...
    df.columns = range(df.shape[1])
        ...
        # ----- 第八欄が空値の行を削除 -----
    Log.Log_Info(Log_File, 'Dropping rows where the 9th column is NaN')
    # Drop all the null data in 8th column, it will keep data clean
    df = df.dropna(subset=[8]).reset_index(drop=True)
    #drop the empty column in the excel file , I think this is new add column
    df = df.drop(columns=[21]).reset_index(drop=True) # test
 
    # ----- 末尾から欠損のデータを落としていく -----
    Getting_Row = len(df) - 1
    # I drpped null data in previous line, so I don't need to check null data again. 
    ...
    while Row_Number < row_end:
        ...
        data_dict = {
            ...
            'key_ICP_Number': df.iloc[Row_Number, Col_ICP_Number],        # new add parameter    
    ...
    Row_Number_Func.next_start_row_number("MESA_StartRow.txt", Next_Start_Row-200)
