SELECT * FROM (
    SELECT
      HDR.STARTTIME,
      HDR.OPERATORID,
      HDR.PARTNUMBER,
      SUBSTR(HDR.SERIALNUMBER,1,5) AS SERIALNUMBER,
      TEQ.DEVICENAME || TEQ.DEVICESERIALNUMBER AS "EQUIPMENT",
      OPS.OPERATIONSTEPNAME || '.' || PAR.PARAMETERNAME AS STEP_AND_PARAMETER,
      MSD.VALUE
    FROM
        TDSMFG.TESTHEADER           HDR,
        TDSMFG.TESTHEADERSTEP       HDS,
        TDSMFG.OPERATIONSTEP        OPS,
        TDSMFG.MEASUREMENTPARAMETER MSP,
        TDSMFG.MEASUREMENT          MSD,
        TDSMFG.PARAMETER            PAR,
        TDSMFG.TESTEQUIPMENT        TEQ
    WHERE 1=1
        AND HDR.TESTHEADERID            = HDS.TESTHEADERID
        AND HDS.OPERATIONSTEPID         = OPS.OPERATIONSTEPID
        AND HDS.TESTHEADERSTEPID        = MSP.TESTHEADERSTEPID
        AND MSP.MEASUREMENTPARAMETERID  = MSD.MEASUREMENTPARAMETERID
        AND MSP.PARAMETERID             = PAR.PARAMETERID
        AND TEQ.TESTHEADERID(+)         = HDR.TESTHEADERID
        AND HDR.OPERATION IN (
            'Dry5_Etching_BiasMNC-loadPosition',
            'Dry5_Etching_BiasMNC-tunePosition',
            'Dry5_Etching_BiasDCProveVoltage',
            'Dry5_Etching_ProcessPressure',
            'Dry5_Etching_BiasRFPower',
            'Dry5_Etching_H2Flow',
            'Dry5_Etching_O2Flow',
            'Dry5_Etching_CH4Flow',
            'Dry5_Etching_BiasReflectedPower',
            'Dry5_Etching_PendulumPosition',
            'Dry5_Etching_ChillerTemp',
            'Dry5_Etching_WallTemp'
        )
    GROUP BY
        STARTTIME,
        OPERATORID,
        PARTNUMBER,
        SERIALNUMBER,
        DEVICENAME,
        DEVICESERIALNUMBER,
        OPERATIONSTEPNAME,
        PARAMETERNAME,
        VALUE
)
PIVOT(
    MAX(VALUE) FOR STEP_AND_PARAMETER IN (
        'BiasMNC-loadPosition.BiasMNC-loadPosition' AS "BiasMNC-loadPosition.BiasMNC-loadPosition",
        'BiasMNC-tunePosition.BiasMNC-tunePosition' AS "BiasMNC-tunePosition.BiasMNC-tunePosition",
        'BiasDCProveVoltage.BiasDCProveVoltage' AS "BiasDCProveVoltage.BiasDCProveVoltage",
        'ProcessPressure.ProcessPressure' AS "ProcessPressure.ProcessPressure",
        'BiasRFPower.BiasRFPower' AS "BiasRFPower.BiasRFPower",
        'H2Flow.H2Flow' AS "H2Flow.H2Flow",
        'O2Flow.O2Flow' AS "O2Flow.O2Flow",
        'CH4Flow.CH4Flow' AS "CH4Flow.CH4Flow",
        'BiasReflectedPower.BiasReflectedPower' AS "BiasReflectedPower.BiasReflectedPower",
        'PendulumPosition.PendulumPosition' AS "PendulumPosition.PendulumPosition",
        'ChillerTemp.ChillerTemp' AS "ChillerTemp.ChillerTemp",
        'WallTemp.WallTemp' AS "WallTemp.WallTemp"
    )
)Int_Data
LEFT JOIN
    (
        SELECT
            MAX(CASE OPERATIONSTEPNAME || '_' || PARAMETERNAME WHEN 'SORTED_DATA_LotNumber_5' THEN VALUESTRING END) AS "FIVE_SERIALNUMBER",
            MAX(CASE OPERATIONSTEPNAME || '_' || PARAMETERNAME WHEN 'SORTED_DATA_LotNumber_9' THEN VALUESTRING END) AS "NINE_SERIALNUMBER"
        FROM
            TDSMFG.TESTHEADER           HDR,
            TDSMFG.TESTHEADERSTEP       HDS,
            TDSMFG.OPERATIONSTEP        OPS,
            TDSMFG.PARAMETER            PAR,
            TDSMFG.STRINGPARAMETER      STP,
            TDSMFG.STRINGMEASUREMENT    SMM
        WHERE 1=1
            AND HDR.TESTHEADERID            = HDS.TESTHEADERID
            AND HDS.OPERATIONSTEPID         = OPS.OPERATIONSTEPID
            AND HDS.TESTHEADERSTEPID        = STP.TESTHEADERSTEPID
            AND STP.STRINGPARAMETERID       = SMM.STRINGPARAMETERID
            AND PAR.OPERATIONSTEPID         = OPS.OPERATIONSTEPID
            AND PAR.PARAMETERID             = STP.PARAMETERID
            AND HDR.OPERATION IN(
            'Dry5_Etching_BiasMNC-loadPosition',
            'Dry5_Etching_BiasMNC-tunePosition',
            'Dry5_Etching_BiasDCProveVoltage',
            'Dry5_Etching_ProcessPressure',
            'Dry5_Etching_BiasRFPower',
            'Dry5_Etching_H2Flow',
            'Dry5_Etching_O2Flow',
            'Dry5_Etching_CH4Flow',
            'Dry5_Etching_BiasReflectedPower',
            'Dry5_Etching_PendulumPosition',
            'Dry5_Etching_ChillerTemp',
            'Dry5_Etching_WallTemp'
        )
        GROUP BY
            HDR.SERIALNUMBER
    )String_Data
    ON Int_Data.SERIALNUMBER = String_Data."FIVE_SERIALNUMBER"
ORDER BY
    STARTTIME
