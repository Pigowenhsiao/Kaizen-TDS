SELECT * FROM (
    ...
    FROM
        ...
        TDSMFG.TESTEQUIPMENT        NAN,
        TDSMFG.TESTEQUIPMENT        DRY
    WHERE 1=1
      ...
      AND NAN.TESTHEADERID(+)           = HDR.TESTHEADERID
      AND DRY.TESTHEADERID(+)           = HDR.TESTHEADERID
      AND HDR.OPERATION                             = 'BJ1_CVD_EtchingRate'
      AND NAN.DEVICENAME                            NOT IN 'DryEtch'
      AND DRY.DEVICENAME || DRY.DEVICESERIALNUMBER  = 'DryEtch#3'
    )
PIVOT (
