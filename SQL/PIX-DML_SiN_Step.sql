SELECT * FROM(
    SELECT
        ...
        PIXPre."PIXPre_STARTTIME",
        PIXPre."PIXPre_Operator",
        PIXPre."PIXPre.Step1",
        PIXPre."PIXPre.Step2",
        PIXPre."PIXPre.Step3",
        PIXPre."PIXPre.Step_Ave",
        PIXPre."PIXPre.Step_3sigma"
        FROM(
                SELECT 
                    ...
                    SELECT * FROM(
                       SELECT
                        ...
                        WHERE 1=1
                            ...
                            AND HDR.OPERATION               = 'PIX-DML_SiN_Step'
                        GROUP BY
                          HDR.TESTHEADERID,
                          PARTNUMBER,
                          SERIALNUMBER,
           ...
           ,(
            ...
            ,(
              SELECT
                    PIXPre_TST.TESTHEADERID,
                    PIXPre_TST."PIXPre_STARTTIME",
                    PIXPre_TST."PIXPre_Operator",
                    PIXPre_TST.PARTNUMBER,
                    PIXPre_TST.SERIALNUMBER,
                    PIXPre_TST."EQUIPMENT",
                    PIXPre_TST."PIXPre.Step1",
                    PIXPre_TST."PIXPre.Step2",
                    PIXPre_TST."PIXPre.Step3",
                    PIXPre_TST."PIXPre.Step_Ave",
                    PIXPre_TST."PIXPre.Step_3sigma"
                 FROM (
                    SELECT * FROM(
                       SELECT
                          HDR.TESTHEADERID,
                          MAX(CASE THM.MISCDESC WHEN  'PIX_Pre_Operator' THEN HDS.STARTDATETIME END) AS "PIXPre_STARTTIME",
                          MAX(CASE THM.MISCDESC WHEN  'PIX_Pre_Operator' THEN THM.MISCVALUE END) AS "PIXPre_Operator",
                          HDR.PARTNUMBER,
                          HDR.SERIALNUMBER,
                          TEQ.DEVICESERIALNUMBER AS EQUIPMENT,
                          OPS.OPERATIONSTEPNAME || '.' || PAR.PARAMETERNAME AS STEP_AND_PARAMETER,
                        ...
                        WHERE 1=1
                            ...
                            AND MSP.PARAMETERID             = PAR.PARAMETERID
                            AND TEQ.TESTHEADERID(+)         = HDR.TESTHEADERID
                            AND THM.MISCDESC                = 'PIX_Pre_Operator'
                            AND HDR.OPERATION               = 'PIX-DML_SiN_Step'
                        GROUP BY
                          HDR.TESTHEADERID,
                          PARTNUMBER,
                ...
                PIVOT(
                    MAX(VALUE) FOR STEP_AND_PARAMETER IN (
                        'PIX_Pre.Step1' AS "PIXPre.Step1",
                        'PIX_Pre.Step2' AS "PIXPre.Step2",
                        'PIX_Pre.Step3' AS "PIXPre.Step3",
                        'PIX_Pre.Step_Ave' AS "PIXPre.Step_Ave",
                        'PIX_Pre.Step_3sigma' AS "PIXPre.Step_3sigma"
                    )
                )
                )PIXPre_TST
            )PIXPre
