SELECT * FROM (
    SELECT
        ...
        HDR.PARTNUMBER,
        HDR.SERIALNUMBER,
        MAX(CASE THM.MISCDESC WHEN 'Process' THEN THM.MISCVALUE END) AS "Process",
        MAX(CASE TEQ.DEVICENAME WHEN 'Nanospec' THEN TEQ.DEVICENAME || TEQ.DEVICESERIALNUMBER END) AS "Nanospec",
        MAX(CASE TEQ.DEVICENAME WHEN 'DryEtch' THEN TEQ.DEVICENAME || TEQ.DEVICESERIALNUMBER END) AS "DryEtch",
    ...
    WHERE 1=1
      ...
      AND OPS.OPERATIONSTEPID        = HDS.OPERATIONSTEPID
      AND MSP.TESTHEADERSTEPID       = HDS.TESTHEADERSTEPID
      AND MSD.MEASUREMENTPARAMETERID = MSP.MEASUREMENTPARAMETERID
      AND MSP.PARAMETERID            = PAR.PARAMETERID
      AND TEQ.TESTHEADERID(+)        = HDR.TESTHEADERID
      AND HDR.OPERATION                     = 'BJ1_CVD_EtchingRate'
