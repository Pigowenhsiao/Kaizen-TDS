...
# Output_filepath = '../XML/031_SEM-EML/'
...
def Open_Data_Sheet(filepath, start_date_time, part_number, serial_number, nine_serial_number):
    ...
    data_dict = {
        ...
        "key_Width_1": sheet.cell(Row_Width_1, 4).value,
        "key_Width_2": sheet.cell(Row_Width_2, 4).value,
        "key_Width_3": sheet.cell(Row_Width_3, 4).value,
        "key_Width_Ave": sheet.cell(Row_Width_Ave, 4).value
    }
...
def Output_XML(xml_file, data_dict):
    ...
    f.write('<?xml version="1.0" encoding="utf-8"?>' + '\n' +
            ...
            '                   <Data DataType="Numeric" Name="Width_1" Units="um" Value=' + '"' + str(data_dict["key_Width_1"]) + '"/>' + '\n' +
            '                   <Data DataType="Numeric" Name="Width_2" Units="um" Value=' + '"' + str(data_dict["key_Width_2"]) + '"/>' + '\n' +
            '                   <Data DataType="Numeric" Name="Width_3" Units="um" Value=' + '"' + str(data_dict["key_Width_3"]) + '"/>' + '\n' +
            '                   <Data DataType="Numeric" Name="Width_Ave" Units="um" Value=' + '"' + str(data_dict["key_Width_Ave"]) + '"/>' + '\n' +
            '               </TestStep>' + '\n' +
...
def main(file_path, start_date, part_number, serial_number, nine_serial_number):
    ...
    # ----- 出力ファイルの確認と削除 -----
    for file in glob.glob(Output_filepath + '*.xml'):
        if os.path.getsize(file) == 0:
            os.remove(file)
            Log.Log_Info(Log_file, f'Removed empty file: {file}')

    buffer_path = '//li.lumentuminc.net/Data/SAG/TDS/Data/XML Buffer/XML Buffer For SPC/'
    for file in glob.glob(buffer_path + '*.xml'):
        if os.path.getsize(file) == 0:
            os.remove(file)
            Log.Log_Info(Log_file, f'Removed empty file from buffer: {file}')