...
# ----- ログ書き込み先ファイルパス -----
Log_file = '../../Log/' + Log_FolderName + '/018_T2-DML_F6-25G.log'
...
# Output_filepath = '../../XML/018_T2-DML/F6-25G/'
...
teststep_dict = {
    ...
    'TestStep20': 'Si2H6-10ppm',
    'TestStep21': 'CH3Cl',
    'TestStep22': 'Temperature',
    'TestStep23': 'Piezocon',
...
########## 前回処理を行ったファイル名を取得する ##########
with open('F6_FileName_F6-25G.txt', 'r', encoding='utf-8') as textfile:
...
def Open_Data_Sheet(Sheet, filepath, SheetName):
    ...
    if conn is None:
        Error_Log(filepath, SheetName, serial_number, "Prime接続エラー")
        sys.exit()
    ...
    data_dict = {
        ...
        "key_Particles_Particles": Sheet['M63'].value,
        ...
        "key_Si2H6-10ppm_Step36" : Sheet["AA35"].value,
        "key_CH3Cl_Step8": Sheet['AB21'].value,
...
def Output_XML(xml_file, data_dict):
    ...
    f.write('<?xml version="1.0" encoding="utf-8"?>' + '\n' +
            ...
            '                   <Data DataType="Numeric" Name="Step36" Units="ppm" Value=' + '"' + str(data_dict["key_Si2H6-10ppm_Step36"]) + '"/>' + '\n' +
            '               </TestStep>' + '\n' +
...
def Data_Extract(filepath, SheetList):
    ...
    for Sheet_Name in SheetList:
        ...
        # ----- シートが処理対象シートかどうか確認 -----
        if '25GDML-HIMEJI-T2.exe' not in str(Sheet['W3'].value) or Initial not in SerialNumber_list:
...
if __name__ == '__main__':
    ...
    # ----- 先ほど処理を行ったファイル名の書き込み -----
    with open('F6_FileName_F6-25G.txt', 'w', encoding='utf-8') as textfile:
        textfile.write(FileName)

    # ----- 最終行を書き込んだファイルをGドライブに転送 -----
    shutil.copy('F6_FileName_F6-25G.txt', 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/018_T2-DML/F6-25G/13_ProgramUsedFile/')
