...
# ----- ログ書き込み先ファイルパス -----
Log_file = '../../Log/' + Log_FolderName + '/008_WG-EML_F2.log'
...
# Output_filepath = '../../XML/008_WG-EML/F2/'
...
########## 前回処理を行ったファイル名を取得する ##########
with open('F2_FileName.txt', 'r', encoding='utf-8') as textfile:
    Before_FileName = textfile.readline()


########## テキストファイルから[Straing_WG]と[Wavelength_WG]の値を取得する ##########
with open('F2_Strain_Wavelength.txt', 'r') as textfile:
...
def Get_Cells_Info(Sheet):
    ...
    # ----- 日付かエピ番号か着工者かバラストN2流量が空欄であれば処理を行わない -----
...
def Output_XML(xml_file, data_dict):
    ...
    f.write('<?xml version="1.0" encoding="utf-8"?>' + '\n' +
            ...
            '                   <Data DataType="Numeric" Name="Step9" Units="slm" Value=' + '"' + str(data_dict["key_PH3-A-50percent_Step9"]) + '"/>' + '\n' +
...
if __name__ == '__main__':
    ...
    if Number[0:5] not in FileName:
        ...
        Data_str = "\n".join(Data_list)
        with open('F2_Strain_Wavelength.txt', 'w', encoding='utf-8') as textfile:
    ...
    # ----- 先ほど処理を行ったファイル名の書き込み -----
    with open('F2_FileName.txt', 'w', encoding='utf-8') as textfile:
        textfile.write(FileName)

    # ----- 各テキストファイルをGドライブに転送 -----
    shutil.copy("F2_Strain_Wavelength.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/008_WG-EML/F2/13_ProgramUsedFile/')
    shutil.copy("F2_FileName.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/008_WG-EML/F2/13_ProgramUsedFile/')
