...
# ----- ログ書き込み先ファイルパス -----
Log_file = '../../Log/' + Log_FolderName + '/008_WG-EML_F1.log'
...
# Output_filepath = '../../XML/008_WG-EML/F1/'
...
teststep_dict = {
    ...
    'TestStep7' : 'MO5-TMI',
    'TestStep8' : 'MO6-TEG',
    'TestStep9' : 'AsH3-5percent',
    'TestStep10' : 'PH3-A-50percent',
    'TestStep11' : 'Temperature',
    'TestStep12' : 'BallastN2',
    'TestStep13' : 'MO-Temperature',
    'TestStep14' : 'Strain',
    'TestStep15' : 'Wavelength',
    'TestStep16' : 'SORTED_DATA',
    'TestStep17' : 'p-InGaAs_Cap', # New add 2025/01/08 cell 73M 
    'TestStep18' : 'PQ_Grating', # New add 2025/01/08 cell 74M 
    'TestStep19' : 'THK_CAP_SIGMA', # New add 2025/01/08 cell L62
    "key_LotNumber_9" : str
}
...
key_type = {
    ...
    "key_MO3-TMI_Step13" : float,
    "key_MO5-TMI_Step21" : float,
    ...
    "key_MO-Temperature_Blank" : float,
    "key_STARTTIME_SORTED": float,
    "key_SORTNUMBER": float,
    "key_THK_CAP_SIGMA" : float, #New add 2025/01/08 cell L62
    "key_PINGAAS_CAP" : float,  #New add 2025/01/08 cell 73M
    "key_PQ_GRATING" : float    #New add 2025/01/08 cell 74M
}
...
########## 前回処理を行ったファイル名を取得する ##########
with open('F1_FileName.txt', 'r', encoding='utf-8') as textfile:
    Before_FileName = textfile.readline()


########## テキストファイルから[Straing_WG]と[Wavelength_WG]の値を取得する ##########
with open('F1_Strain_Wavelength.txt','r') as textfile:
...
def Get_Cells_Info(Sheet):
    ...
    if Sheet['I8'].value is None or Sheet['Q7'].value is None or Sheet['AE42'].value is None:
        is_cells_empty = True
...
def Open_Data_Sheet(Sheet, filepath, SheetName):
    ...
    data_dict = {
        ...
        "key_Thickness_Thickness_Cap": Sheet["M46"].value,
        "key_Thickness_Thickness_Core": Sheet["M47"].value,
        "key_Thickness_Thickness_Total": Sheet["M48"].value,
        ...
        "key_MO2-TEG_Step17" : Sheet["L23"].value,
        "key_MO3-TMI_Step13" : Sheet["M22"].value,
        "key_MO5-TMI_Step21" : Sheet["O24"].value,
        ...
        "key_Temperature_Step9" : Sheet["Z21"].value,
        "key_Temperature_Step13" : Sheet["Z22"].value,
        "key_Temperature_Step17" : Sheet["Z23"].value,
        "key_Temperature_Step21" : Sheet["Z24"].value,
        ...
        "key_MO-Temperature_Blank" : Sheet["Q42"].value,
        "key_PINGAAS_CAP" : Sheet["M73"].value, #New add 2025/01/08 cell 73M
        "key_PQ_GRATING" : Sheet["M74"].value,   #New add 2025/01/08 cell 74M
        "key_THK_CAP_SIGMA" : Sheet["L62"].value, #New add 2025/01/08 cell L62
        "key_SEM": Equipment1,
...
def Output_XML(xml_file, data_dict):
    ...
    f.write('<?xml version="1.0" encoding="utf-8"?>' + '\n' +
            ...
            '                   <Data DataType="Numeric" Name="Step13" Units="sccm" Value=' + '"' + str(data_dict["key_MO3-TMI_Step13"]) + '"/>' + '\n' +
            '               </TestStep>' + '\n' +
            '               <TestStep Name=' + '"' + teststep_dict["TestStep7"] + '"' + ' startDateTime=' + '"' + data_dict["key_start_date_time"] + '"' + ' Status="Passed">' + '\n' +
            '                   <Data DataType="Numeric" Name="Step21" Units="sccm" Value=' + '"' + str(data_dict["key_MO5-TMI_Step21"]) + '"/>' + '\n' +
            '               </TestStep>' + '\n' +
            ...
            '                   <Data DataType="Numeric" Name="Blank" Units="degree" Value=' + '"' + str(data_dict["key_MO-Temperature_Blank"]) + '"/>' + '\n' +
            '               </TestStep>' + '\n' +
            ...
            '                   <Data DataType="Numeric" Name="PIN_GaAs_Cap" Units="nm" Value=' + '"' + str(data_dict["key_PINGAAS_CAP"]) + '"/>' + '\n' +
            '               </TestStep>' + '\n' +
            '               <TestStep Name=' + '"' + teststep_dict["TestStep18"] + '"' + ' startDateTime=' + '"' + data_dict["key_start_date_time"] + '"' + ' Status="Passed">' + '\n' +
            '                   <Data DataType="Numeric" Name="PQ_Grating" Units="nm" Value=' + '"' + str(data_dict["key_PQ_GRATING"]) + '"/>' + '\n' +
            '               </TestStep>' + '\n' +
            '               <TestStep Name=' + '"' + teststep_dict["TestStep19"] + '"' + ' startDateTime=' + '"' + data_dict["key_start_date_time"] + '"' + ' Status="Passed">' + '\n' +
            '                   <Data DataType="Numeric" Name="THK_CAP_SIGMA" Units="nm" Value=' + '"' + str(data_dict["key_THK_CAP_SIGAMA"]) + '"/>' + '\n' +
            '               </TestStep>' + '\n' +
...
def Data_Extract(filepath, SheetList, old_check):
    ...
    for Sheet_Name in Sheets:
        ...
        # ----- シートが処理対象シートかどうか確認 -----
        if '13B2-WG.exe' not in str(Sheet['U3'].value):
            Log.Log_Error(Log_file, Sheet_Name + ' : ' + 'Not Covered\n')
            continue

        if 'ﾁｪｯｸ' in str(Sheet['M8'].value) or 'チェック' in str(Sheet['M8'].value):
            # ----- 多層CHで抜き出す値を定義 -----
            Data_list[0] = str(Sheet['M46'].value)
            Data_list[1] = str(Sheet['M49'].value)
            Data_list[2] = str(Sheet['M50'].value)
            Data_list[3] = str(Sheet['M51'].value)
        ...
        if Initial not in SerialNumber_list:
            print("INIT",Initial,"SER:",SerialNumber_list)
            Log.Log_Error(Log_file, Sheet_Name + ' : ' + 'Not Covered\n')
            continue
        ...
        # ----- [Straing_WG]と[Wavelength]を辞書に追加する -----
...
if __name__ == '__main__':
    ...
    if Number not in FileName:
        ...
        Data_str = "\n".join(Data_list)
        with open('F1_Strain_Wavelength.txt', 'w', encoding='utf-8') as textfile:
    ...
    # ----- 先ほど処理を行ったファイル名の書き込み -----
    with open('F1_FileName.txt', 'w', encoding='utf-8') as textfile:
        textfile.write(FileName)

    # ----- 各テキストファイルをGドライブに転送 -----
    shutil.copy("F1_Strain_Wavelength.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/008_WG-EML/F1/13_ProgramUsedFile/')
    shutil.copy("F1_FileName.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/008_WG-EML/F1/13_ProgramUsedFile/')
