...
# ----- ログ書き込み先ファイルパス -----
Log_file = '../../Log/' + Log_FolderName + '/008_WG-EML_F9.log'
...
########## 前回処理を行ったファイル名を取得する ##########
with open('F9_FileName.txt', 'r', encoding='utf-8') as textfile:
...
def Open_Data_Sheet(Sheet, filepath, SheetName, dummy_row, row_number):
    ...
    Equipment1, Equipment2, Equipment3, Equipment4 = '1', '1', '1', '9'
...
def Data_Extract(filepath, SheetList, old_check):
    ...
    for Sheet_Name in SheetList:
        ...
        # ----- シートがXML変換対象シートか確認 -----
        if '-WG' not in str(Sheet['U4'].value):
...
if __name__ == '__main__':
    ...
    if Number not in FileName:
        ...
        Old_File_Path = MOCVD_OldFileSearch.F9(Number);
    ...
    # ----- 先ほど処理を行ったファイル名の書き込み -----
    with open('F9_FileName.txt', 'w', encoding='utf-8') as textfile:
        textfile.write(FileName)

    # ----- 各テキストファイルをGドライブに転送 -----
    #shutil.copy("F9_FileName.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/008_WG-EML/F9/13_ProgramUsedFile/')
