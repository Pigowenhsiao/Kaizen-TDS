...
Log_File = '../Log/' + Log_Folder_Name + '/027_MESA.log'
...
Col_Depth_Depth_4 = 8
Col_Depth_Depth_5 = 9
Col_Depth_Depth_Ave = 10
Col_Depth_Depth_3sigma = 11
...
key_type = {
    ...
    'key_Part_Number' : str,
    'key_Equipment' : str,
    'key_Depth_Depth_1' : float,
    'key_Depth_Depth_2' : float,
    'key_Depth_Depth_3' : float,
    'key_Depth_Depth_4' : float,
    'key_Depth_Depth_5' : float,
    'key_Depth_Depth_Ave' : float,
    'key_Depth_Depth_3sigma' : float,
...
def Main():
    ...
    FilePath = 'Z:/■QCデータシート/ウェットエッチ/'
    FileName = '*ﾒｻｴｯﾁ深さ*.xls*'
    Excel_File_List = []
    ...
    Excel_File = shutil.copy(Excel_File_List[0][0], '../DataFile/027_MESA/')
    ...
    df = pd.read_excel(Excel_File, header=None, sheet_name=Data_Sheet_Name, usecols="B:M", skiprows=Start_Number)
    ...
    while Row_Number < row_end:
        ...
        data_dict = {
            ...
            'key_Depth_Depth_1': df.iloc[Row_Number, Col_Depth_Depth_1],
            'key_Depth_Depth_2': df.iloc[Row_Number, Col_Depth_Depth_2],
            'key_Depth_Depth_3': df.iloc[Row_Number, Col_Depth_Depth_3],
            'key_Depth_Depth_4': df.iloc[Row_Number, Col_Depth_Depth_4],
            'key_Depth_Depth_5': df.iloc[Row_Number, Col_Depth_Depth_5],
            'key_Depth_Depth_Ave': df.iloc[Row_Number, Col_Depth_Depth_Ave],
            'key_Depth_Depth_3sigma': df.iloc[Row_Number, Col_Depth_Depth_3sigma]
        }
        ...
        f.write('<?xml version="1.0" encoding="utf-8"?>' + '\n' +
                ...
                '                   <Data DataType="Numeric" Name="Depth_1" Units="nm" Value=' + '"' + str(data_dict["key_Depth_Depth_1"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Depth_2" Units="nm" Value=' + '"' + str(data_dict["key_Depth_Depth_2"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Depth_3" Units="nm" Value=' + '"' + str(data_dict["key_Depth_Depth_3"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Depth_4" Units="nm" Value=' + '"' + str(data_dict["key_Depth_Depth_4"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Depth_5" Units="nm" Value=' + '"' + str(data_dict["key_Depth_Depth_5"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Depth_Ave" Units="nm" Value=' + '"' + str(data_dict["key_Depth_Depth_Ave"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Depth_3sigma" Units="nm" Value=' + '"' + str(data_dict["key_Depth_Depth_3sigma"]) + '"/>' + '\n' +
                '               </TestStep>' + '\n' +
                '\n'
                ...
                '\n'
                '               <TestEquipment>' + '\n' +
                '                   <Item DeviceName="Dektak" DeviceSerialNumber="' + str(data_dict['key_Equipment']).replace('＃', '#') + '"/>' + '\n' +
                '               </TestEquipment>' + '\n' +
                '\n'
                '               <ErrorData/>' + '\n' +
                '               <FailureData/>' + '\n' +
    ...
    shutil.copy("MESA_StartRow.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/027_MESA/13_ProgramUsedFile/')
