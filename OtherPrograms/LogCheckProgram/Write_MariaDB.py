...
Log_Paths = ["../../Log/", "../../001_QC/Log/", "../../002_Enginnering/Log/"]
Log_List = []
...
for Log_Path in Log_Paths:
    ...
    Log_List.extend(glob.glob(Path + "/*.Log"))

# ----- プログラムの開始時間/終了時間を得る -----

# 更新日時が一番早いファイルと遅いファイルのパスを取り出す
Time_FileName = [[time.localtime(os.path.getmtime(File)), File] for File in Log_List]
Time_FileName.sort()
Fast_File, Slow_File = Time_FileName[0][1], Time_FileName[-1][1]


# Fast_FileからはProgram Startを見つけ、その中で一番遅い時間のものを探す
