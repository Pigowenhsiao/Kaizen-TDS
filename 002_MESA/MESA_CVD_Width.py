...
config.read('MESA_CVD_WIDTH.ini', encoding='utf-8')

# 基本參數
Site = config.get('Basic_info', 'Site')
ProductFamily = config.get('Basic_info', 'ProductFamily')
Operation = config.get('Basic_info', 'Operation')
TestStation = config.get('Basic_info', 'TestStation')
FileName = config.get('Basic_info', 'file_name_patterns')
retention_date = int(config.get('Basic_info', 'retention_date'))

# 路徑參數
FilePath = config.get('Paths', 'input_paths')
Output_filepath = config.get('Paths', 'output_path')
intermediate_data_path = config.get('Paths', 'intermediate_data_path')
...
if not os.path.exists(os.path.join(log_path, Log_Folder_Name)):
    os.makedirs(os.path.join(log_path, Log_Folder_Name))

Log_File = os.path.join(log_path, Log_Folder_Name, '002_MESA.log')
Log.Log_Info(Log_File, 'Program Start')



# Excel 相關參數
Data_Sheet_Name = config.get('Excel', 'sheet_name')
data_columns = config.get('Excel', 'data_columns')
main_skip_rows = int(config.get('Excel', 'main_skip_rows'))
main_dropna_key_col_idx = int(config.get('Excel', 'main_dropna_key_col_idx'))
serial_number_source_column_idx = int(config.get('Excel', 'serial_number_source_column_idx'))
XY_Sheet_Name = config.get('Excel', 'xy_sheet_name')
xy_columns = config.get('Excel', 'xy_columns')
xy_coord_x_col_idx = int(config.get('Excel', 'xy_coord_x_col_idx'))
xy_coord_y_col_idx = int(config.get('Excel', 'xy_coord_y_col_idx'))
xy_num_points = int(config.get('Excel', 'xy_num_points'))
EA_DML_Sheet_Name = '品名一覧'  # 若 INI 有定義可改
...
# 取得欄位索引 (可根據 INI DataFields 解析)
Col_Start_Date_Time = 0
Col_Operator = 1
Col_Sem_Number = 2
Col_Serial_Number = serial_number_source_column_idx
Col_ICP_Number = 5
Col_Width1 = 12
Col_Width2 = 13
Col_Width3 = 14
...
key_type = {
    ...
    "key_ICP_Number": str,
    "key_Litho_Etch_Tool": str, # Add 2025-02-28 yoshida
    "key_Operator": str,
    "key_Width1": float,
    "key_Width2": float,
    "key_Width3": float,
...
def Main():
    ...
    Excel_File = shutil.copy(
        Excel_File_List[0][0],
        os.path.join(intermediate_data_path, os.path.basename(Excel_File_List[0][0]))
    )
    ...
    Start_Number = Row_Number_Func.start_row_number(running_rec) - main_skip_rows

    # ----- ExcelデータをDataFrameとして取得 -----
    Log.Log_Info(Log_File, 'Read Excel')
    df = pd.read_excel(Excel_File, header=None, sheet_name=Data_Sheet_Name, usecols=data_columns, skiprows=Start_Number)
    df_EA_DML = pd.read_excel(Excel_File, header=None, sheet_name=EA_DML_Sheet_Name, usecols="H:P", skiprows=5)
    df_xy = pd.read_excel(Excel_File, header=None, sheet_name=XY_Sheet_Name, usecols=xy_columns)
    ...
    while Row_Number < row_end:
        ...
        data_dict = {
            ...
            "key_Litho_Etch_Tool": parse_litho_etch_tool_info(df.iloc[Row_Number, Col_ICP_Number]), # Add 2025-02-28 yoshida
            ...
            "key_Width_Average": df.iloc[Row_Number, Col_WidthAve],
        ...
        f.write('<?xml version="1.0" encoding="utf-8"?>' + '\n' +
                ...
                '               <Header SerialNumber=' + '"' + data_dict["key_Serial_Number"] + '"' +
                                        ' PartNumber=' + '"' + data_dict["key_Part_Number"] + '"' +
                                        ' Operation=' + '"' + Operation + '"' +
                                        ' TestStation=' + '"' + TestStation + '"' +
                                        ' Operator=' + '"' + data_dict["key_Operator"] + '"' +
                                        ' StartTime=' + '"' + data_dict["key_Start_Date_Time"].replace(".", ":") + '"' +
                                        ' Site=' + '"' + Site + '"' + ' LotNumber=' + '"' + data_dict["key_Serial_Number"] +  '"' +
                                        ' MISCINFO=' + '"ICP' + data_dict["key_ICP_Number"]+ '"'
                                '/>'+
                        '"/>' + '\n' +
                '\n'
                ...
                '               </TestStep>' + '\n' +
                '               <TestStep Name="WidthAve" startDateTime=' + '"' + data_dict["key_Start_Date_Time"].replace(".", ":") + '"' + ' Status=' + '"' + tmp_Result + '">' + '\n' +
                '                   <Data DataType="Numeric" Name="Width" Units="nm" Value=' + '"' + str(data_dict["key_Width_Average"]) + '"/>' + '\n' +
                '               </TestStep>' + '\n' +
                ...
                '                   <Data DataType="String" Name="Litho_Etch_Tool" Value=' + '"' + str(data_dict["key_Litho_Etch_Tool"]) + '"' + ' CompOperation="LOG"/>' + '\n' + # Add 2025-02-28 yoshida 
                '               </TestStep>' + '\n' +
                '\n'
                '               <TestEquipment>' + '\n' +
...
# Add 2025-02-28 yoshida
def parse_litho_etch_tool_info(text):
    #Example
    #    input    ->    output

    #    '#1'    ->    'EB_ICP1'
    #    'ステッパ#2 , ICP#2'    ->    'Stepper2_ICP2'
    ...
    if len(result)==1:
        ...
        if ("ステッパ" in result[0]) or ("ｽﾃｯﾊﾟ" in result[0]):
            if "1" in result[1]:
                return "Stepper2_ICP1"
            elif "2" in result[1]:
                return "Stepper2_ICP2"
            else:
                return None
        elif ("EB" in result[0]) or ("ＥＢ" in result[0]):
            if "1" in result[1]:
                return "EB_ICP1"
            elif "2" in result[1]:
                return "EB_ICP2"
