...
########## 自作関数の定義 ##########
from F3_Module import F3_ICP
from F3_Module import F3_RIE
...
########## 処理が完了したシート名一覧を取得する ##########
with open('F3_Before_FileName.txt', 'r', encoding = 'utf-8') as textfile:
    Before_FileName = textfile.readline()


########## 多層CHの値を取得 ##########
with open('F3_Multi_CH.txt','r') as textfile:
...
def Data_Extract(filepath, SheetName):
    ...
    for Sheet_Name in SheetName[::-1]:
        ...
        if 'dT-HL13B7B8' in str(Sheet['U4'].value) or 'dT-HL13B4B5' in str(Sheet['U4'].value):
            F3_ICP.main(filepath, Sheet_Name, Data_list)
            continue

        # ----- RIEシートか -----
        if '13B2RU-BH-CUP' in str(Sheet['U4'].value):
            F3_RIE.main(filepath, Sheet_Name, Data_list)
            continue
...
if __name__ == '__main__':
    ...
    for filename in all_files:
        ...
        if "FE" in str(filename) and os.path.isfile(filepath) and "$" not in str(filename):
    ...
    if Number not in FileName:
        ...
        # ----- 対象ファイルパスを返す -----
        Old_File_Path = MOCVD_OldFileSearch.F3(Number)
        ...
        # ----- 多層CHの値の書き込み -----
        Multi_CH_str = "\n".join(Data_list)
        with open('F3_Multi_CH.txt', 'w', encoding='utf-8') as textfile:
    ...
    # ----- 先ほど処理を行ったファイル名の書き込み -----
    with open('F3_Before_FileName.txt', 'w', encoding='utf-8') as textfile:
