...
key_type = {
    ...
    "key_MO7-TMI_Step11" : float,
    "key_MO6-Ru_Step10" : float,
    "key_MO6-Ru_Step11" : float,
    ...
    "key_PH3-A-50percent_Step11" : float,
    "key_CH3Cl-5percent_Step7" : float,
    "key_CH3Cl-5percent_Step10" : float,
    "key_CH3Cl-5percent_Step11" : float,
...
def Get_Cells_Info(file_path, SheetName):
    ...
    if Sheet['I8'].value is None or \
        ...
        Sheet['AD46'].value is None: #AD46 ﾊﾞﾗｽﾄN2流量
        is_cells_empty = True
...
def Open_Data_Sheet(file_path, SheetName, Multi_CH_List):
    ...
    # ----- 異物数判定 -----
    if sheet['M50'].value is None and sheet["AD54"] is None: #M50 異物数
        Particles = ""
    elif sheet['M50'].value is not None:
        Particles = sheet['M50'].value
    else:
        Particles = sheet['AD54'].value #AD54 異物数
    ...
    data_dict = {
        ...
        "key_BallastN2_BallastN2" : sheet['AD46'].value,
        "key_MO-Temperature_MO7-TMI" : sheet["K44"].value,
        "key_MO-Temperature_MO8-Fe" : sheet["L44"].value,
        "key_MO-Temperature_MO6-Ru" : sheet["M44"].value,
        ...
        "key_MO7-TMI_Step10" : sheet['K24'].value,
        "key_MO7-TMI_Step11" : sheet['K25'].value,
        "key_MO6-Ru_Step10" : sheet['M24'].value,
        "key_MO6-Ru_Step11" : sheet['M25'].value,
        "key_PH3-A-50percent_Step7" : sheet['T21'].value,
        "key_PH3-A-50percent_Step8" : sheet['T22'].value,
        "key_PH3-A-50percent_Step9" : sheet['T23'].value,
        "key_PH3-A-50percent_Step10" : sheet['T24'].value,
        "key_PH3-A-50percent_Step11" : sheet['T25'].value,
        "key_CH3Cl-5percent_Step7" : sheet['W21'].value,
        "key_CH3Cl-5percent_Step10" : sheet['W24'].value,
        "key_CH3Cl-5percent_Step11" : sheet['W25'].value,
        ...
        "key_TMInPiezoconConc_TMInPiezoconConc_ZeroValue" : sheet["AA33"].value,
        "key_TMInPiezoconConc_TMInPiezoconConc_Check" : sheet["AA34"].value,
        "key_TMInPiezoconConc_TMInPiezoconConc_EPI" : sheet["AA35"].value,
        "key_TMInPiezoconConc_TMInPiezoconConc_CheckFlow" : sheet["AA37"].value,
        "key_TMInPiezoconConcOffset_TMInPiezoconConcOffset_Check" : sheet["AD34"].value,
        "key_TMInPiezoconConcOffset_TMInPiezoconConcOffset_EPI" : sheet["AD35"].value,
        "key_TMInPiezoconConcOffset_TMInPiezoconConcOffset_CheckFlow" : sheet["AD37"].value,
        "key_CH3Cl_Conc_CH3Cl_ConcBonbe" : sheet['AD39'].value,
...
def Output_XML(xml_file, data_dict):
    ...
    f.write('<?xml version="1.0" encoding="utf-8"?>' + '\n' +
            ...
            '                   <Data DataType="Numeric" Name="Step11" Units="sccm" Value=' + '"' + str(data_dict["key_MO7-TMI_Step11"]) + '"/>' + '\n' +
            '               </TestStep>' + '\n' +
            ...
            '                   <Data DataType="Numeric" Name="Step11" Units="sccm" Value=' + '"' + str(data_dict["key_MO6-Ru_Step11"]) + '"/>' + '\n' +
            '               </TestStep>' + '\n' +
            ...
            '                   <Data DataType="Numeric" Name="Step11" Units="sccm" Value=' + '"' + str(data_dict["key_PH3-A-50percent_Step11"]) + '"/>' + '\n' +
            '               </TestStep>' + '\n' +
            ...
            '                   <Data DataType="Numeric" Name="Step11" Units="sccm" Value=' + '"' + str(data_dict["key_CH3Cl-5percent_Step11"]) + '"/>' + '\n' +
            '               </TestStep>' + '\n' +
...
def main(file_path, SheetName, Multi_CH_List):
    ...
    # ----- 位置NG発生番地の入力チェック -----
    if type(data_dict["key_Particles_Particles_NG_Address"]) == str:
        if len(data_dict["key_Particles_Particles_NG_Address"]) > 2:
            for item in data_dict["key_Particles_Particles_NG_Address"].split(','):
                if len(item) != 2:
                    Log.Log_Error(Log_File, data_dict["key_serial_number"] + ' : ' + "Illegal Particles NG Address \n")
                    return
