...
Operation = 'N-electrode_N-ISO_Width'
...
Log_File = '../Log/' + Log_Folder_Name + '/030_N-electrode.log'
Log.Log_Info(Log_File, 'Program Start')


########## シート名の定義 ##########
Data_Sheet_Name = 'QC-K-017-R01'


########## XML出力先ファイルパス ##########
Output_filepath = '//li.lumentuminc.net/data/SAG/TDS/Data/Files to Insert/XML/'
# Output_filepath = '../XML/030_N-electrode/'
Output_filepath = 'C:/Users/hsi67063/Downloads/TEMP/'
...
Col_Operator = 2
Col_Equipment = 4
Col_Width_Center_L = 5
Col_Width_Center_R = 6
Col_Width_Outer_L = 7
Col_Width_Outer_R = 8
Col_Width_Ave = 9
Col_Width_MAX = 10
...
key_type = {
    ...
    'key_Part_Number': str,
    'key_Equipment': str,
    'key_Width_Center_L': float,
    'key_Width_Center_R': float,
    'key_Width_Outer_L': float,
    'key_Width_Outer_R': float,
    'key_Width_Ave': float,
    'key_Width_Max': float,
    'key_Width_Min': float,
...
def Main():
    ...
    FileName = '*N-ISO仕上り寸法データシート*.xlsx'
    Excel_File_List = []
    ...
    Excel_File = shutil.copy(Excel_File_List[0][0], '../DataFile/030_N-electrode/')
    ...
    Start_Number = Row_Number_Func.start_row_number("N-electrode_StartRow.txt") - 500

    # ----- ExcelデータをDataFrameとして取得 -----
    Log.Log_Info(Log_File, 'Read Excel')
    df = pd.read_excel(Excel_File, header=None, sheet_name=Data_Sheet_Name, usecols="B:N", skiprows=Start_Number)
    ...
    for i in range(df.shape[0]):
        if type(df.iloc[i, 1]) is not pd.Timestamp:
            df.iloc[i, 1] = np.nan
    ...
    while Row_Number < row_end:
        ...
        data_dict = {
            ...
            'key_Equipment': '#' + str(df.iloc[Row_Number, Col_Equipment])[0],
            'key_Width_Center_L': df.iloc[Row_Number, Col_Width_Center_L],
            'key_Width_Center_R': df.iloc[Row_Number, Col_Width_Center_R],
            'key_Width_Outer_L': df.iloc[Row_Number, Col_Width_Outer_L],
            'key_Width_Outer_R': df.iloc[Row_Number, Col_Width_Outer_R],
            'key_Width_Ave': df.iloc[Row_Number, Col_Width_Ave],
            'key_Width_Max': df.iloc[Row_Number, Col_Width_MAX],
            'key_Width_Min': df.iloc[Row_Number, Col_Width_MIN],
        }
        ...
        f.write('<?xml version="1.0" encoding="utf-8"?>' + '\n' +
                ...
                '                   <Data DataType="Numeric" Name="Center_L" Units="um" Value=' + '"' + str(data_dict["key_Width_Center_L"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Center_R" Units="um" Value=' + '"' + str(data_dict["key_Width_Center_R"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Outer_L" Units="um" Value=' + '"' + str(data_dict["key_Width_Outer_L"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Outer_R" Units="um" Value=' + '"' + str(data_dict["key_Width_Outer_R"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Ave" Units="um" Value=' + '"' + str(data_dict["key_Width_Ave"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="MAX" Units="um" Value=' + '"' + str(data_dict["key_Width_Max"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="MIN" Units="um" Value=' + '"' + str(data_dict["key_Width_Min"]) + '"/>' + '\n' +
                '               </TestStep>' + '\n' +
                '\n'
                ...
                '\n'
                '               <TestEquipment>' + '\n' +
                '                   <Item DeviceName="Microscope" DeviceSerialNumber="' + str(data_dict['key_Equipment']) + '"/>' + '\n' +
                '               </TestEquipment>' + '\n' +
                '\n'
                '               <ErrorData/>' + '\n' +
                '               <FailureData/>' + '\n' +
    ...
    Row_Number_Func.next_start_row_number("N-electrode_StartRow.txt", Start_Number + row_end)

    # ----- 最終行を書き込んだファイルをGドライブに転送 -----
    shutil.copy("N-electrode_StartRow.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/030_N-electrode/13_ProgramUsedFile/')
