...
def process_ini_file(config_path):
    ...
    log_file = os.path.join(log_folder_path, '003_Polish.log')
    ...
    def process_excel_file(file_path):
        ...
        while row_number < row_end:
            ...
            try:
                part_number, nine_serial_number = SQL.selectSQL(cursor, df.iloc[row_number, 4])
            except Exception as e:
    ...
    for input_path in input_paths:
        ...
        for file in files:
            if not os.path.basename(file).startswith('~$'):
                destination_dir = '../DataFile/Polished_XML/'
                if not os.path.exists(destination_dir):
                    os.makedirs(destination_dir)
                shutil.copy(file, destination_dir)
                Log.Log_Info(global_log_file, f"Copy excel file {file} to ../DataFile/Polished_XML/")
