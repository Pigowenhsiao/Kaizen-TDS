...
Output_filepath  = 'C:/Users/hsi67063/Downloads/'  # 別の可能なXML出力パス（コメントアウト）
...
key_type = {
    ...
    "key_PolishAVG": float,  # ポリッシュAVGの型  2025/01/10 追加
    "key_X1": float,  # X1座標の型
    "key_X2": float,  # X2座標の型
    "key_X3": float,  # X3座標の型
    "key_X4": float,  # X4座標の型
    ...
    try:
        with open(Output_filepath + XML_File_Name, 'w', encoding="utf-8") as f:  # XMLファイルを開いて書き込み
            f.write('<?xml version="1.0" encoding="utf-8"?>\n' +
                    ...
                    generate_test_steps(data_dict) + generate_test_avg(data_dict) + # 生成 TestStep1 から TestStep13
...
def generate_test_avg(data_dict):
    """生成 TestStep 部分の XML 内容"""
    test_steps_AVG =  f'               <TestStep Name="Thickness_AVG" startDateTime="{data_dict["key_Start_Date_Time"].replace(".", ":")}" Status="Done">\n' + \
                      f'                   <Data DataType="Numeric" Name="Thickness" Units="um" Value="{data_dict[f"key_PolishAVG"]}"/>\n' + \
                      f'               </TestStep>\n'
    return test_steps_AVG
