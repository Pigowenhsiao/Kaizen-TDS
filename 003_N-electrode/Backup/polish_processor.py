...
class PolishProcessor:
    ...
    def process_file(self, filepath: str, config: Dict[str, Any], workbook: openpyxl.Workbook = None):
        """
        處理單一檔案
        Args:
            filepath (str): Excel 文件路徑
            config (dict): 操作的配置字典
            workbook (openpyxl.Workbook, optional): 已載入的 Excel 工作簿。
        """
        try:
            data = self._extract_data(filepath, config, workbook)
            if data:
                self._generate_xml(data, config)
        except Exception as e:
            logging.error(f"Failed to process file {filepath}: {e}")

    def _extract_data(self, filepath: str, config: Dict[str, Any], workbook: openpyxl.Workbook) -> Dict[str, Any]:
        """
        從 Excel 提取資料
        Args:
            filepath (str): Excel 文件路徑
            config (dict): 配置字典
            workbook (openpyxl.Workbook): 已載入的 Excel 工作簿。
        Returns:
            dict: 提取的資料
        """

        logging.info(f"Extracting data from {filepath} for {config['operation']}")
        sheet = workbook[config["data_sheet_name"]]
        xy_sheet = workbook[config["xy_sheet_name"]]
