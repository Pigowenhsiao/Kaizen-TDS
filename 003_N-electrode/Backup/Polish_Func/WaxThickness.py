...
Operation = 'N-electrode_Polish_WaxThickness'  # 操作名を定義
...
Row_Start_Date_Time = 4  # 開始日時が存在する行番号（0ベースなのでExcelの5行目）
Row_Operator = 5  # オペレーターが存在する行番号（0ベースなのでExcelの6行目）
Row_Polish = 19  # ポリッシュ厚みデータが存在する行番号（0ベースなのでExcelの20行目）
...
key_type = {
    ...
    "key_LotNumber_9": str,  # LotNumber_9は文字列型
...
def Open_Data_Sheet(filepath, Part_Number, Nine_Serial_Number):
    ...
    # 取得序列號、開始日期時間和操作者
    try:
        Start_Date_Time = sheet.cell(row=Row_Start_Date_Time + 1, column=3).value  # Get start date time from row=5, column=3 (C5)
        Serial_Number = sheet.cell(row=Row_Serial_Number + 1, column=3).value  # Get serial number from row=4, column=3 (C4)
        Operator = sheet.cell(row=Row_Operator + 1, column=3).value  # Get operator from row=6, column=3 (C6)
    except Exception as e:
    ...
    Polish = []
    try:
        for i in range(13):  # 取得13個打磨數據
            Polish.append(sheet.cell(row=Row_Polish + 1, column=4 + i).value)  # 第20行第4到16列（D20 to P20）
    except Exception as e:
    ...
    try:
        ...
        x, y = [], []
        for i in range(1, 14):  # 取得13個X/Y座標
...
def generate_test_steps(data_dict):
    """生成 TestStep 部分の XML 內容"""
    test_steps = ""
    for i in range(1, 14):  # 生成 13 個 TestStep 元素
        test_steps += (
            f'               <TestStep Name="Thickness{i}" startDateTime="{data_dict["key_Start_Date_Time"].replace(".", ":")}" Status="Done">\n' +  # TestStep 要素を属性付きで書き込む
    ...
    return test_steps  # 生成した TestStep を返す
