...
Log_File = '../Log/' + Log_Folder_Name + '/019_MESA.log'
...
Col_Width_4 = 14
Col_Width_5 = 15
Col_WidthAve_Average = 16
Col_WidthAve_3sigma = 17
...
def Main():
    ...
    FileName = '*QC-K-012-R07*BNKﾎﾄ後ﾒｻ寸法*.xls*'
    Excel_File_List = []
    ...
    Excel_File = shutil.copy(Excel_File_List[0][0], '../DataFile/019_MESA/')
    ...
    df = pd.read_excel(Excel_File, header=None, sheet_name=Data_Sheet_Name, usecols="B:S", skiprows=Start_Number)
    ...
    shutil.copy("MESA_StartRow.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/019_MESA/13_ProgramUsedFile/')
