...
def detect_tool_name(filename, tool_map):
    """Detects tool name based on filename ."""
...
def process_excel_file(filepath_str, settings, log_file, csv_filepath):
    ...
    try:
        ...
        # Step 2: Conditionally read the XY coordinate worksheet (Wafer Location mode)
        xy_data = {}
        if settings.xy_sheet_name:
            Log.Log_Info(log_file, f"Wafer location mode detected. Reading XY coordinate sheet: '{settings.xy_sheet_name}'")
    ...
    rename_map = {}
    special_renames = {'key_Serial_Number': 'Serial_Number', 'key_Part_Number': 'Part_Number', 'key_Start_Date_Time': 'Start_Date_Time'}
