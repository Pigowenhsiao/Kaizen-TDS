...
Operation_Thickness = 'P-electrode_DML_Deposition_Thickness'
Operation_Stress = 'P-electrode_DML_Deposition_Stress'
TestStation = 'P-electrode'
TestStep_Thickness = 'Thickness'
TestStep_Stress = 'Stress'
...
Log_File = '../Log/' + Log_Folder_Name + '/022_P-electrode.log'
...
#Output_filepath = '../XML/022_P-electrode/'
...
########## XML作成 ##########
def Output_XML(xml_file, data_dict, Main_data, Operation, TestStep):

    # ----- Result判定 -----
    if TestStep == 'Thickness':
        Result = 'Done'
    else:
        if data_dict['key_Result'] == '合':
            Result = 'Passed'
        else:
            Result = 'Failed'
    ...
    f.write('<?xml version="1.0" encoding="utf-8"?>' + '\n' +
            ...
            '       <Result startDateTime=' + '"' + data_dict["key_Start_Date_Time"].replace(".", ":") + '"' + ' Result=' + '"' + Result + '">' + '\n' +
            ...
            #'               <Header SerialNumber=' + '"' + data_dict["key_Serial_Number"] + '"' + ' PartNumber=' + '"' + data_dict["key_Part_Number"] + '"' + ' Operation=' + '"' + Operation + '"' + ' TestStation=' + '"' + TestStation + '"' + ' Operator=' + '"' + data_dict["key_Operator"] + '"' + ' StartTime=' + '"' + data_dict["key_Start_Date_Time"].replace(".", ":") + '"' + ' Site=' + '"' + Site + '"' + ' LotNumber=' + '"' + data_dict["key_Serial_Number"] + '"/>' + '\n' +
            '               <HeaderMisc>' + '\n' +
            '                   <Item Description="' + TestStep +'">' + HeaderMisc + '</Item>' + '\n'
            '               </HeaderMisc>' + '\n' +
            '\n'
            '               <TestStep Name="' + TestStep + '" startDateTime=' + '"' + data_dict["key_Start_Date_Time"].replace(".", ":") + '"' + ' Status=' + '"' + Result + '">' + '\n' +
            '                   <Data DataType="Numeric" Name="' + TestStep + '" Units="' + Units + '" Value=' + '"' + str(Main_data) + '"/>' + '\n' +
            '               </TestStep>' + '\n' +
            '\n'
            ...
            '\n'
            '               <TestEquipment>' + '\n' +
            '                   <Item DeviceName="EB vapor deposition" DeviceSerialNumber=' + '"' + str(data_dict["key_Equipment"]) + '"/>' + '\n' +
            '               </TestEquipment>' + '\n' +
            '\n'
            '               <ErrorData/>' + '\n' +
            '               <FailureData/>' + '\n' +
...
def Main():
    ...
    FilePath = 'Z:/■QCデータシート/電極・研磨/'
    FileName = 'QC-K-009-R04*電極応力測定データシート*.xls*'
    Excel_File_List = []
    ...
    Excel_File = shutil.copy(Excel_File_List[0][0], '../DataFile/022_P-electrode/')
    ...
    Start_Number = Row_Number_Func.start_row_number("P-electrode_DML_StartRow.txt") - 500

    # ----- ExcelデータをDataFrameとして取得 -----
    Log.Log_Info(Log_File, 'Read Excel')
    df = pd.read_excel(Excel_File, header=None, sheet_name=Data_Sheet_Name, usecols="B:V", skiprows=Start_Number)
    ...
    # ----- 作業日/作業者/装置Noのnanは前の値を参照する -----
    df[Col_Start_Date_Time] = df[Col_Start_Date_Time].ffill()
    df[Col_Operator] = df[Col_Operator].ffill()
    df[Col_Equipment] = df[Col_Equipment].ffill()
    #df[Col_Serial_Number] = df[Col_Serial_Number].fillna(method='ffill')


    # ----- 次の開始行数をメモ -----
    Next_Start_Row = Start_Number + df.shape[0] + 1
    try:
        # Handle dates with 'E' by removing the 'E' before conversion
        df[0] = df[0].apply(lambda x: str(x).replace('E', '') if isinstance(x, str) else x)
        df[0] = pd.to_datetime(df[0])
    except Exception as e:
        Log.Log_Error(Log_File, f"DateTime conversion error: {e}")
    ...
    while Row_Number < row_end:
        ...
        data_dict = {
            ...
            'key_Equipment' : df.iloc[Row_Number, Col_Equipment],
            'key_Thickness' : df.iloc[Row_Number, Col_Thickness],
            'key_Stress' : df.iloc[Row_Number, Col_Stress],
        ...
        # ----- Thickness -----
        XML_File_Name = 'Site=' + Site + ',ProductFamily=' + ProductFamily + ',Operation=' + Operation_Thickness + \
                        ',Partnumber=' + Part_Number + ',Serialnumber=' + data_dict["key_Serial_Number"] + \
                        ',Testdate=' + data_dict["key_Start_Date_Time"] + '.xml'
        Output_XML(XML_File_Name, data_dict, data_dict['key_Thickness'], Operation_Thickness, TestStep_Thickness)

        # ----- Stress -----
        XML_File_Name = 'Site=' + Site + ',ProductFamily=' + ProductFamily + ',Operation=' + Operation_Stress + \
                        ',Partnumber=' + Part_Number +',Serialnumber=' + data_dict["key_Serial_Number"] + \
                        ',Testdate=' +  data_dict["key_Start_Date_Time"] + '.xml'
        Output_XML(XML_File_Name, data_dict, data_dict['key_Stress'], Operation_Stress, TestStep_Stress)
        Log.Log_Info(Log_File, 'XML File Created\n' + Output_filepath + XML_File_Name)
...
#',Partnumber=' + data_dict["key_Part_Number"] + ',Serialnumber=' + data_dict["key_Serial_Number"] + \
    ...
    Row_Number_Func.next_start_row_number("P-electrode_DML_StartRow.txt", Next_Start_Row)

    # ----- 最終行を書き込んだファイルをGドライブに転送 -----
    shutil.copy("P-electrode_DML_StartRow.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/022_P-electrode/13_ProgramUsedFile/')
    #shutil.copy("P-electrode_DML_StartRow.txt",
    #            'C:/Users/fus86274/Documents/fuse/0010_Kaizen-TDS_開発/test/01_開発/022_P-electrode/DML/')
