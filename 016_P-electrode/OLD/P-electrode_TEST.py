...
Operation_Thickness = 'P-electrode_EML_Deposition_Thickness'
Operation_Stress = 'P-electrode_EML_Deposition_Stress'
...
Log_File = '../Log/' + Log_Folder_Name + '/016_P-electrode.log'
...
# Output_filepath = '../XML/016_P-electrode/'
...
def Main():
    ...
    FileName = 'QC-K-009-R04*電極応力測定データシート*.xls*' #fixed the name of the file
    Excel_File_List = []
    ...
    Excel_File = shutil.copy(Excel_File_List[0][0], '../DataFile/016_P-electrode/')
    ...
    Start_Number = Row_Number_Func.start_row_number("P-electrode_EA_StartRow.txt") - 500
    ...
    for i in range(df.shape[0]):
        if df.iloc[i,Col_Equipment] == "nan":
            df.iloc[i,Col_Equipment] = df.iloc[i-1,Col_Equipment]
            print(df.iloc[i,Col_Equipment])
    ...
    while Row_Number < row_end:
        ...
        if data_dict["key_Equipment"] is np.nan:
            data_dict["key_Equipment"] = ""
    ...
    Row_Number_Func.next_start_row_number("P-electrode_EA_StartRow.txt", Next_Start_Row)

    # ----- 最終行を書き込んだファイルをGドライブに転送 -----
    shutil.copy("P-electrode_EA_StartRow.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/016_P-electrode/13_ProgramUsedFile/')
