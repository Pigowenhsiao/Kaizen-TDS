...
Operation = 'TH-DML_MESA-Side_Step'
TestStation = 'TH-DML'
...
key_type = {
    ...
    "key_Equipment" : str,
    'key_Step_Step1': float,
    'key_Step_Step2': float,
    'key_Step_Step3': float,
    'key_Step_Step4': float,
...
def Main():
    ...
    FilePath = 'Z:/ホト・エッチング/製品/ﾄﾞﾗｲｴｯﾁ3号機/3ｲﾝﾁ/直変系/'
    FileName = '*3インチ_直変系_THSiO2膜ドライエッチプログラムシート*.xlsm'
    Excel_File_List = []
    ...
    Excel_File = shutil.copy(Excel_File_List[0][0], '../DataFile/028_TH-DML/')
    ...
    Start_Number = Row_Number_Func.start_row_number("TH-DML_StartRow.txt") - 500

    # ----- ExcelデータをDataFrameとして取得 -----
    Log.Log_Info(Log_File, 'Read Excel')
    df = pd.read_excel(Excel_File, header=None, sheet_name=Data_Sheet_Name, usecols="E:R", skiprows=Start_Number)

    # ----- 使わない列を落とす -----
    df = df.drop(range(8,14), axis=1)

    # ----- 列番号の振り直し -----
    Log.Log_Info(Log_File, 'Setting Columns Number')
    df.columns = range(df.shape[1])

    # ----- 00:00:00をnp.nanに置き換える -----
    ...
    while Row_Number < row_end:
        ...
        data_dict = {
            ...
            'key_Step_Step1': df.iloc[Row_Number, Col_Step_Step1],
            'key_Step_Step2': df.iloc[Row_Number, Col_Step_Step2],
            'key_Step_Step3': df.iloc[Row_Number, Col_Step_Step3],
            'key_Step_Step4': df.iloc[Row_Number, Col_Step_Step4]
        }
        ...
        f.write('<?xml version="1.0" encoding="utf-8"?>' + '\n' +
                ...
                '                   <Data DataType="Numeric" Name="Step1" Units="nm" Value=' + '"' + str(data_dict["key_Step_Step1"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Step2" Units="nm" Value=' + '"' + str(data_dict["key_Step_Step2"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Step3" Units="nm" Value=' + '"' + str(data_dict["key_Step_Step3"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Step4" Units="nm" Value=' + '"' + str(data_dict["key_Step_Step4"]) + '"/>' + '\n' +
                '               </TestStep>' + '\n' +
                '\n'
    ...
    Row_Number_Func.next_start_row_number("TH-DML_StartRow.txt", Next_Start_Row)

    # ----- 最終行を書き込んだファイルをGドライブに転送 -----
    shutil.copy("TH-DML_StartRow.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/028_TH-DML/13_ProgramUsedFile/')
