...
Operation = 'P-electrode_SiN_Step'
...
Log_File = '../Log/' + Log_Folder_Name + '/015_P-electrode_SiN.Log'
...
# Output_filepath = '../XML/015_P-electrode/SiN/'
...
key_type = {
    ...
    'key_SiN_Step2': float,
    'key_SiN_Step3': float,
    'key_SiN_Step_Ave': float,
    'key_SiN_Step_3sigma': float,
...
def Main():
    ...
    Start_Number = Row_Number_Func.start_row_number("SiN_StartRow.txt") - 500

    # ----- ExcelデータをDataFrameとして取得 -----
    Log.Log_Info(Log_File, 'Read Excel')
    df = pd.read_excel(Excel_File, header=None, sheet_name=Data_Sheet_Name, usecols="C:L", skiprows=Start_Number)
    ...
    while Row_Number < row_end:
        ...
        data_dict = {
            ...
            'key_SiN_Step1': df.iloc[Row_Number, Col_SiN_Step1],
            'key_SiN_Step2': df.iloc[Row_Number, Col_SiN_Step2],
            'key_SiN_Step3': df.iloc[Row_Number, Col_SiN_Step3],
            'key_SiN_Step_Ave': df.iloc[Row_Number, Col_SiN_Step_Ave],
            'key_SiN_Step_3sigma': df.iloc[Row_Number, Col_SiN_Step_3sigma],
        ...
        f.write('<?xml version="1.0" encoding="utf-8"?>' + '\n' +
                ...
                '                   <Data DataType="Numeric" Name="Step1" Units="nm" Value=' + '"' + str(data_dict["key_SiN_Step1"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Step2" Units="nm" Value=' + '"' + str(data_dict["key_SiN_Step2"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Step3" Units="nm" Value=' + '"' + str(data_dict["key_SiN_Step3"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Step_Ave" Units="nm" Value=' + '"' + str(data_dict["key_SiN_Step_Ave"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Step_3sigma" Units="nm" Value=' + '"' + str(data_dict["key_SiN_Step_3sigma"]) + '"/>' + '\n' +
                '               </TestStep>' + '\n' +
                ...
                '\n'
                '               <TestEquipment>' + '\n' +
                '                   <Item DeviceName="Assher" DeviceSerialNumber="' + str(data_dict["key_Equipment"]) + '"/>' + '\n' +
                '               </TestEquipment>' + '\n' +
                '\n'
                '               <ErrorData/>' + '\n' +
                '               <FailureData/>' + '\n' +
    ...
    Row_Number_Func.next_start_row_number("SiN_StartRow.txt", Next_Start_Row)

    # ----- 最終行を書き込んだファイルをGドライブに転送 -----
    shutil.copy("SiN_StartRow.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/015_P-electrode/13_ProgramUsedFile/')
