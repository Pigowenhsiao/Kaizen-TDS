...
Operation = 'N-electrode_DML_LiftOffPattern_Width'
...
Log_File = '../Log/' + Log_Folder_Name + '/036_N-electrode_DML_LiftOffPattern_Width.log'
Log.Log_Info(Log_File, 'Program Start')


########## 使用するシート名と開始行数を取り出すファイルを定義 ##########
Sheet_Text_Array = [
    ["HL13BFCP-BT", "HL13BFCP_StartRow_PW.txt"],
    ["HTL139S・HL1361_2_3", "HTL139S_etc_StartRow_PW.txt"],
    ["HL13BE・BS･CE・CG", "HL13BE_etc_StartRow_PW.txt"]
]


########## XML出力先ファイルパス ##########
Output_filepath = '//li.lumentuminc.net/data/SAG/TDS/Data/Files to Insert/XML/'
# Output_filepath = '../XML/036_N-electrode/'
...
Col_Operator = 1
Col_Equipment = 2
Col_Pattern_Width_Ave = 6
Col_Pattern_Width_Max = 7
Col_Pattern_Width_Min = 8
Col_Serial_Number = 9
...
key_type = {
    ...
    "key_Operator": str,
    "key_Pattern_Width_Ave": float,
    "key_Pattern_Width_Max": float,
    "key_Pattern_Width_Min": float,
...
def Main(Data_Sheet_Name, TextFile):
    ...
    Excel_File = shutil.copy(Excel_File_List[0][0], '../DataFile/036_N-electrode/')
    ...
    df = pd.read_excel(Excel_File, header=None, sheet_name=Data_Sheet_Name, usecols="T:AB", skiprows=Start_Number)
    Serial_df = pd.read_excel(Excel_File, header=None, sheet_name=Data_Sheet_Name, usecols="E", skiprows=Start_Number)

    # ----- dfとSerial_dfを連結 -----
    df = pd.concat([df, Serial_df], axis=1)
    ...
    while Row_Number < row_end:
        ...
        # ----- 品種が 10G-DFB-LD もしくは 25G-DFB のときに処理を継続 -----
        if Part_Class_Check(Part_Number) != '10G-DFB-LD' and Part_Class_Check(Part_Number) != '25G-DFB':
        ...
        data_dict = {
            ...
            "key_Pattern_Width_Ave": df.iloc[Row_Number, Col_Pattern_Width_Ave],
            "key_Pattern_Width_Max": df.iloc[Row_Number, Col_Pattern_Width_Max],
            "key_Pattern_Width_Min": df.iloc[Row_Number, Col_Pattern_Width_Min]
        }
        ...
        f.write('<?xml version="1.0" encoding="utf-8"?>' + '\n' +
                ...
                '                   <Data DataType="Numeric" Name="Ave" Units="um" Value=' + '"' + str(data_dict["key_Pattern_Width_Ave"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Max" Units="um" Value=' + '"' + str(data_dict["key_Pattern_Width_Max"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="Min" Units="um" Value=' + '"' + str(data_dict["key_Pattern_Width_Min"]) + '"/>' + '\n' +
                '               </TestStep>' + '\n' +
                '\n'
    ...
    shutil.copy(TextFile, 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/036_N-electrode/13_ProgramUsedFile/')


if __name__ == '__main__':

    for SheetName, TextFile in Sheet_Text_Array:
        Main(SheetName, TextFile)
