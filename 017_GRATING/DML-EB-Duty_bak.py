...
Log_File = '../Log/' + Log_Folder_Name + '/017_GRATING.log'
...
Output_filepath = '../XML/017_GRATING/'
...
Col_Duty2_2 = 21
Col_Duty2_3 = 22
Col_DutyAve_Average = 23
Col_DutyAve_3sigma = 24
...
key_type = {
    ...
    'key_Duty2_2': float,
    'key_Duty2_3': float,
    'key_DutyAve_Average': float,
    'key_DutyAve_3sigma': float,
...
def Main():
    ...
    FileName = '*QC-D-001-R03*EBﾚｼﾞｽﾄ回折格子品質*.xls*'
    Excel_File_List = []
    ...
    Excel_File = shutil.copy(Excel_File_List[0][0], '../DataFile/017_GRATING/')
    ...
    Start_Number = Row_Number_Func.start_row_number("GRATING_StartRow.txt") - 500
    ...
    while Row_Number < row_end:
        ...
        data_dict = {
            ...
            'key_Duty1_1' : df.iloc[Row_Number, Col_Duty1_1],
            'key_Duty1_2' : df.iloc[Row_Number, Col_Duty1_2],
            'key_Duty1_3' : df.iloc[Row_Number, Col_Duty1_3],
            'key_Duty2_1' : df.iloc[Row_Number, Col_Duty2_1],
            'key_Duty2_2' : df.iloc[Row_Number, Col_Duty2_2],
            'key_Duty2_3' : df.iloc[Row_Number, Col_Duty2_3],
            'key_DutyAve_Average' : df.iloc[Row_Number, Col_DutyAve_Average],
            'key_DutyAve_3sigma' : df.iloc[Row_Number, Col_DutyAve_3sigma],
        }
        ...
        f.write('<?xml version="1.0" encoding="utf-8"?>' + '\n' +
                ...
                '                   <Data DataType="Numeric" Name="1-1" Units="AU" Value=' + '"' + str(data_dict["key_Duty1_1"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="1-2" Units="AU" Value=' + '"' + str(data_dict["key_Duty1_2"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="1-3" Units="AU" Value=' + '"' + str(data_dict["key_Duty1_3"]) + '"/>' + '\n' +
                '               </TestStep>' + '\n' +
                '\n'
                '               <TestStep Name="Duty2" startDateTime=' + '"' + data_dict["key_Start_Date_Time"].replace(".", ":") + '"' + ' Status="Passed">' + '\n' +
                '                   <Data DataType="Numeric" Name="2-1" Units="AU" Value=' + '"' + str(data_dict["key_Duty2_1"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="2-2" Units="AU" Value=' + '"' + str(data_dict["key_Duty2_2"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="2-3" Units="AU" Value=' + '"' + str(data_dict["key_Duty2_3"]) + '"/>' + '\n' +
                '               </TestStep>' + '\n' +
                '\n'
                '               <TestStep Name="DutyAve" startDateTime=' + '"' + data_dict["key_Start_Date_Time"].replace(".", ":") + '"' + ' Status="Passed">' + '\n' +
                '                   <Data DataType="Numeric" Name="Average" Units="AU" Value=' + '"' + str(data_dict["key_DutyAve_Average"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="3sigma" Units="AU" Value=' + '"' + str(data_dict["key_DutyAve_3sigma"]) + '"/>' + '\n' +
                '               </TestStep>' + '\n' +
                '\n'
    ...
    Row_Number_Func.next_start_row_number("GRATING_StartRow.txt", Next_Start_Row)

    # ----- 最終行を書き込んだファイルをGドライブに転送 -----
    #shutil.copy("GRATING_StartRow.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/017_GRATING/13_ProgramUsedFile/')
    shutil.copy("GRATING_StartRow.txt",
                'C:/Users/fus86274/Documents/fuse/0010_Kaizen-TDS_開発/test/01_開発/017_GRATING/')
