...
PIX3_Serial_Numbers = set()  # PIX3 的 Serial number 列表
...
def Main(FilePath, FileNamePattern, TextFile, PIX):
    ...
    while Row_Number < row_end:
        ...
        if PIX == "PIX1":
            ...
            if PIX != "PIX3" and Serial_Number not in PIX3_Serial_Numbers:
                Log.Log_Error(LOG_FILE, Serial_Number + ' : ' + "Not in PIX3 Serial Numbers\n")
                Row_Number += 1
                continue
...
# 4. PIX3 の場合、XML ファイルへの変換出力処理を実施
    if PIX == "PIX3":
        ...
        if Last_index != float('inf'):
            ...
            for pix_type in ["PIX1", "PIX2", "PIX3"]:
                shutil.copy(f"{pix_type}_StartRow.txt", start_row_file_base)
