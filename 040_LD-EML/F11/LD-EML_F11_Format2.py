# LD-EML_F11_Format2.py (based on LD-EML_F10_Format2.py) 2025-05-08 yoshida
...
import warnings
warnings.simplefilter(action='ignore', category=UserWarning) #ログが巨大になるので拡張子警告を出力しない。問題調査のときはコメントアウト
...
# ------ 入力ファイルパス ------
input_filepath = [
                  "//sagqnap01.li.lumentuminc.net/K11Data/PL-MAP#2/★PLﾏｯﾊﾟｰ判定/HL13B5/F11LD/",
                  "//sagqnap01.li.lumentuminc.net/K11Data/PL-MAP#3/★PLﾏｯﾊﾟｰ判定/HL13B5/F11LD/",
                  "//sagqnap01.li.lumentuminc.net/K11Data/PL-MAP#3/★PLﾏｯﾊﾟｰ判定/4インチ_HL13B5/F11LD/",
                  ]
...
def missing_epinum_or_lotnum(file_path):
    ...
    # by using pd.read_excel(instead of direct use of xlrd/openpyxl), both xls and xlsm(xlsx) can be supported.
    df = pd.read_excel(file_path, sheet_name=Data_sheet_name, header=None)

    if (df.iloc[2, 7] is None) or (df.iloc[3, 7] is None): # [2,7]=Epi No. , [3,7]=Lot No.
        is_cells_empty = True
    else:
...
#        if str(File)[:2] != 'PL' or str(File)[-7:] != 'LD.xlsm':
        if str(File)[:2] != 'PL' or str(File)[-5:] != '.xlsm':
                continue
