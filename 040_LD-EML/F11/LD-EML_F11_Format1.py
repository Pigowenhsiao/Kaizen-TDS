# LD-EML_F11_Format1.py (based on LD-EML_F10_Format1.py)
# items not in PQC list deleted
...
########## 全体パラメータの定義 ##########
MOCVD_Equipment = 'F11'
input_file_prefix = 'F11プログラムシート'

Site = '350'
ProductFamily = 'SAG FAB'
Operation = 'LD-EML_' + MOCVD_Equipment + '_Format1'
TestStation = 'LD-EML'
X = '999999'
Y = '999999'
...
# ----- ログ書き込み先ファイルパス -----
Log_File = '../../Log/' + Log_FolderName + '/040_LD-EML_' + MOCVD_Equipment + '_Format1.log'
...
########## 処理ファイルのあるディレクトリ定義 ##########
Path = 'Z:/MOCVD/MOCVD過去プログラム/' + MOCVD_Equipment + '炉/'
# Path = '../../InputFolder/' # debug
...
########## 前回処理を行ったファイル名を取得する ##########
with open('./' + MOCVD_Equipment + '_FileName_Format1.txt', 'r', encoding='utf-8') as textfile:
...
def Open_Data_Sheet(Sheet, filepath, Sheet_Name):
    ...
    SEM, XRD, MOCVD = 'NoData', 'NoData', MOCVD_Equipment
    ...
    data_dict = {
        ...
        "key_HeaderMisc1": Sheet["V3"].value,       # modify from F1
        "key_HeaderMisc2": Sheet["V4"].value,       # modify from F1
        "key_HeaderMisc3": Sheet["V5"].value,       # modify from F1
        ...
        "key_XRayDiffraction_Xray_Thickness": Sheet['M46'].value,       # modify from F1
        "key_XRayDiffraction_Xray_Strain": Sheet['M47'].value,          # modify from F1
        "key_Adjust_PL_Wavelength": Sheet['L52'].value,       # modify from F1
        "key_Adjust_PL_Intensity": Sheet['N52'].value,       # modify from F1
        "key_Adjust_PL_FWHM": Sheet['P52'].value,       # modify from F1
        "key_Adjust_PL_Adjust": Sheet['L54'].value,       # modify from F1
        ...
        "key_MeasurementConditions_PL_IntensityRate_A": Sheet['AD58'].value,       # modify from F1
        "key_MeasurementConditions_PL_IntensityRate_B": Sheet['AD59'].value,       # modify from F1
        "key_MeasurementConditions_PL_IntensityRate_C": Sheet['AD60'].value,       # modify from F1
        "key_MeasurementConditions_PL_IntensityRate_D": Sheet['AD61'].value,       # modify from F1
...
def Data_Extract(filepath, SheetList, old_check):
    ...
    for Sheet_Name in SheetList[::-1]:
        ...
        # F10 programsheet has no recipe info, so deleted condition about recipe
        #if Initial not in SerialNumber_list or ('13B2-LD' not in str(Sheet['U3'].value) and '13B4-LD' not in str(Sheet['U3'].value) and '13B5-LD' not in str(Sheet['U3'].value)):
        if (Initial not in SerialNumber_list) or ('13B5' not in str(Sheet['A4'].value)):
...
if __name__ == '__main__':
    ...
    for filename in all_files:
        ...
        if input_file_prefix in str(filename) and '$' not in str(filename) and os.path.isfile(filepath):
    ...
    # ----- 先ほど処理を行ったファイル名の書き込み -----
    with open('./' + MOCVD_Equipment + '_FileName_Format1.txt', 'w', encoding='utf-8') as textfile:
        textfile.write(FileName)

########## ログ書込：プログラムの終了 ##########
Log.Log_Info(Log_File, 'Program End' + Operation)