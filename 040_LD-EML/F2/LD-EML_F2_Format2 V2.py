...
def Get_Cells_Info(file_path):
    ...
    if file_path.lower().endswith('.xls'):
        ...
        # xlrd 使用 0-indexed：第3列為 row=2，第4列為 row=3；第8欄為 col=7
        is_cells_empty = (sheet.cell(2, 7).value is None or sheet.cell(3, 7).value is None)
        wb.release_resources()
    else:
        wb = px.load_workbook(file_path, data_only=True)
        sheet = wb[Data_sheet_name]
        # openpyxl 使用 1-indexed：第3列為 row=3，第4列為 row=4；第8欄為 column=8
        is_cells_empty = (sheet.cell(row=3, column=8).value is None or sheet.cell(row=4, column=8).value is None)
        wb.close()
        print("A:",sheet.cell(row=3, column=8).value,"B:",sheet.cell(row=4, column=8).value)
...
def Open_Data_Sheet(file_path):
    ...
    if file_path.lower().endswith('.xls'):
        ...
        data_dict['Lamnda_diff'] = data_dict["key_Mapper_Average_Mapper_PL_Lambda"] - data_dict["key_Mapper_Adjust_Mapper_Wavelength(1.3um)"]
        data_dict['Lamnda2_diff'] = data_dict["key_Mapper_Average_Mapper_PL_Lambda2"] - data_dict["key_Center_Lambda"]
        wb.release_resources()
    else:
        wb = px.load_workbook(file_path, data_only=True)
        sheet = wb[Data_sheet_name]
...
if __name__ == '__main__':
    ...
    # 指定需要處理的路徑
    Path = [ "Z:/PL-MAP/★PLﾏｯﾊﾟｰ判定/3ｲﾝﾁHTL13B5/F2 LD/", "Z:/PL-MAP#2/★PLﾏｯﾊﾟｰ判定/HL13B5/F2 LD/" ]
    for p in Path:
        ALL_FILE_FETCH(p)
        
    for FilePath in ALL_FILES:
        file_mod_time = datetime.fromtimestamp(os.path.getmtime(FilePath))
        if (datetime.now() - file_mod_time).days > 60:
