...
class IniSettings:
    ...
    def __init__(self):
        ...
        self.skip_rows = 500
        self.field_map = {}
        self.main_dropna_key_col = "key_Serial_Number"
        # CVD-specific
        self.tool_name = ""
        # ICP/Dry-specific
        self.xy_sheet_name = ""
        self.xy_columns = ""
        self.xy_sheet_is_optional = False # New feature
        self.tool_name_map = {}
...
def _extract_settings_from_config(config):
    ...
    s.main_dropna_key_col = config.get('Excel', 'main_dropna_key_col', fallback='key_Serial_Number')
    ...
    s.xy_sheet_is_optional = config.getboolean('Excel', 'xy_sheet_is_optional', fallback=False) # New feature
...
def process_excel_file(filepath_str, settings, log_file, csv_filepath):
    ...
    try:
        ...
        Log.Log_Info(log_file, f"Step 1: Successfully read main sheet '{settings.sheet_name}', {df.shape[0]} rows loaded starting from row {start_row}.")
        df.columns = list(range(df.shape[1]))
        # Map column indices to key names defined in the INI
        ini_keys_by_col_index = {int(v['col']): k for k, v in settings.field_map.items() if v['col'].isdigit()}
        df.rename(columns=ini_keys_by_col_index, inplace=True)
        
        # Step 2: Conditionally read the XY coordinate worksheet (ICP/Dry mode)
        xy_data = {}
        if settings.xy_sheet_name:
            Log.Log_Info(log_file, f"ICP/Dry mode detected. Attempting to read XY sheet: '{settings.xy_sheet_name}'")
            try:
                ...
                Log.Log_Info(log_file, "XY coordinate data parsed successfully.")
            except ValueError as e: # Catches errors like "Worksheet not found"
                if settings.xy_sheet_is_optional:
                    Log.Log_Info(log_file, f"Optional XY sheet not found: '{settings.xy_sheet_name}'. Continuing without XY data. Details: {e}")
                else:
                    Log.Log_Error(log_file, f"Required XY sheet not found: '{settings.xy_sheet_name}'. Aborting process for this file. Details: {e}")
                    raise # Re-raise the exception to stop processing
        else:
            Log.Log_Info(log_file, "No XY coordinate sheet setting detected. Processing in CVD mode.")

        # Step 3: Initial filtering based on Date and the key indicator column
        date_format = '%Y-%m-%d %H:%M:%S'
        date_series = pd.to_datetime(df.get('key_Start_Date_Time'), format=date_format, errors='coerce')
        df = df[date_series.notna() & (date_series >= (datetime.now() - relativedelta(days=settings.retention_date)))]
        
        Log.Log_Info(log_file, f"Using '{settings.main_dropna_key_col}' as the key column for dropping empty rows.")
        if settings.main_dropna_key_col in df.columns:
            df.dropna(subset=[settings.main_dropna_key_col], inplace=True)
        else:
            Log.Log_Info(log_file, f"Warning: main_dropna_key_col '{settings.main_dropna_key_col}' not found in DataFrame columns after initial read. Skipping dropna.")
        
        Log.Log_Info(log_file, f"Step 2: Initial filtering (date, key column) complete. {df.shape[0]} valid rows remaining.")
    ...
    special_renames = {
        ...
        'key_TestEquipment_SEM': 'SEM_DeviceSerialNumber',
    ...
    # Build order from field_map to respect INI order
    ...
    # Step 8: Update the starting row record using the new, robust logic
    if not df.empty:
        last_processed_index = df.index[-1]
        next_start_row = last_processed_index + 1
        
        Log.Log_Info(log_file, f"Last processed row index was {last_processed_index}. Updating next start row to {next_start_row}")
        ...
        Log.Log_Info(log_file, "No valid data was processed, running_rec will not be updated.")
...
def main():
    ...
    if not ini_files:
        ...
        return
    Log.Log_Info(log_file, f"Found {len(ini_files)} config file(s): {', '.join(map(os.path.basename, ini_files))}")
    ...
    for ini_path in ini_files:
        try:
            ...
            print(error_message)
            if 'log_file' in locals(): Log.Log_Error(log_file, error_message)

    init_log_file = setup_logging('../Log/', 'UniversalScript_Init')
    Log.Log_Info(init_log_file, "===== Universal Script End =====")
