...
def Main():
    ...
    #Excel_File = '../DataFile/009_GRATING/QC-E-025-R04　EA-DFB  EBﾚｼﾞｽﾄ.xlsx'
    ...
    df = df.drop(columns=[35])  # remove column AJ (index 35)
    df = df[df[7].notna()].reset_index(drop=True)  # remove rows where column H (index 7) is NaN
    ...
    for i in range(df.shape[0]):
        print(df.iloc[i, 3])
        if type(df.iloc[i, 3]) is not pd.Timestamp:
            df.iloc[i, 3] = np.nan
            if i <=20:
    ...
    while Row_Number < row_end:
        ...
        if df.isnull().sum(axis=1)[df_idx[Row_Number]] > 1:
        ...
        try:
            ...
            Log.Log_Error(Log_File, Serial_Number + ' : ' + 'SQL Query Failed - ' + str(e))
