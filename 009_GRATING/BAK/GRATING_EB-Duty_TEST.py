...
Log_File = '../Log/' + Log_Folder_Name + '/009_GRATING.log'
...
Output_filepath = r'C:\Users\hsi67063\Downloads\\'
# Output_filepath = '../XML/009_GRATING/'


########## 取得するデータの列番号を定義 ##########
Col_Sem_Number = 1 
Col_Serial_Number = 2 
Col_Start_Date_Time = 3 
Col_Operator = 4 
Col_Duty1_A = 11 
Col_Duty1_B = 12
Col_Duty1_C = 13
...
def Main():
    ...
    Excel_File = shutil.copy(Excel_File_List[0][0], '../DataFile/009_GRATING/')
    ...
    Start_Number = Row_Number_Func.start_row_number("GRATING_EB_StartRow.txt") - 500

    # ----- ExcelデータをDataFrameとして取得 -----
    Log.Log_Info(Log_File, 'Read Excel')
    df = pd.read_excel(Excel_File, header=None, sheet_name=Data_Sheet_Name, usecols="C:AK", skiprows=Start_Number)
    ...
    for i in range(df.shape[0]):
        if type(df.iloc[i, 2]) is not pd.Timestamp:
            df.iloc[i, 2] = np.nan
            if i <=20:
                print('date:',df.iloc[i, 2])
    ...
    while Row_Number < row_end:
        ...
        ########## 空欄判定 ##########
        # ----- 空欄が1つでも存在すればTrueを返す -----
        if pd.isnull(df.iloc[df_idx[Row_Number], 32]):
            # Fill NaN value with 0 in the 32nd column of the current row
            df.iloc[df_idx[Row_Number], 32] = 0

        # ----- 空欄が1つでも存在すればTrueを返す -----
        Log.Log_Info(Log_File, "Blank Check")
        if df.isnull().any(axis=1)[df_idx[Row_Number]]:
            Log.Log_Info(Log_File, 'Blank error')
            # Find the columns with null values
            null_columns = df.columns[df.iloc[df_idx[Row_Number]].isnull()].tolist()
            Log.Log_Info(Log_File, f'Null columns: {null_columns}')
            Row_Number += 1
            continue
        ...
        data_dict = {
            ...
            'key_Duty1_A': df.iloc[Row_Number, Col_Duty1_A],
            'key_Duty1_B': df.iloc[Row_Number, Col_Duty1_B],
            'key_Duty1_C': df.iloc[Row_Number, Col_Duty1_C],
            'key_Duty2_A': df.iloc[Row_Number, Col_Duty2_A],
            'key_Duty2_B': df.iloc[Row_Number, Col_Duty2_B],
            'key_Duty2_C': df.iloc[Row_Number, Col_Duty2_C],
            'key_Duty3_A': df.iloc[Row_Number, Col_Duty3_A],
            'key_Duty3_B': df.iloc[Row_Number, Col_Duty3_B],
            'key_Duty3_C': df.iloc[Row_Number, Col_Duty3_C],
            'key_Duty_Average': df.iloc[Row_Number, Col_Duty_Average],
            'key_Duty_3sigma': df.iloc[Row_Number, Col_Duty_3sigma]
        }

        print('data_dic:',data_dict)
        ########## 日付フォーマットの変換 ##########
        ...
        f.write('<?xml version="1.0" encoding="utf-8"?>' + '\n' +
                ...
                '\n'
                '               <TestStep Name="Duty1" startDateTime=' + '"' + data_dict["key_Start_Date_Time"].replace(".", ":") + '"' + ' Status="Passed">' + '\n' +
                '                   <Data DataType="Numeric" Name="A" Units="AU" Value=' + '"' + str(data_dict["key_Duty1_A"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="B" Units="AU" Value=' + '"' + str(data_dict["key_Duty1_B"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="C" Units="AU" Value=' + '"' + str(data_dict["key_Duty1_C"]) + '"/>' + '\n' +
                '               </TestStep>' + '\n' +
                '               <TestStep Name="Duty2" startDateTime=' + '"' + data_dict["key_Start_Date_Time"].replace(".", ":") + '"' + ' Status="Passed">' + '\n' +
                '                   <Data DataType="Numeric" Name="A" Units="AU" Value=' + '"' + str(data_dict["key_Duty2_A"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="B" Units="AU" Value=' + '"' + str(data_dict["key_Duty2_B"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="C" Units="AU" Value=' + '"' + str(data_dict["key_Duty2_C"]) + '"/>' + '\n' +
                '               </TestStep>' + '\n' +
                '               <TestStep Name="Duty3" startDateTime=' + '"' + data_dict["key_Start_Date_Time"].replace(".", ":") + '"' + ' Status="Passed">' + '\n' +
                '                   <Data DataType="Numeric" Name="A" Units="AU" Value=' + '"' + str(data_dict["key_Duty3_A"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="B" Units="AU" Value=' + '"' + str(data_dict["key_Duty3_B"]) + '"/>' + '\n' +
                '                   <Data DataType="Numeric" Name="C" Units="AU" Value=' + '"' + str(data_dict["key_Duty3_C"]) + '"/>' + '\n' +
                '               </TestStep>' + '\n' +
                '               <TestStep Name="DutyAve" startDateTime=' + '"' + data_dict["key_Start_Date_Time"].replace(".", ":") + '"' + ' Status="Passed">' + '\n' +
                '                   <Data DataType="Numeric" Name="Average" Units="AU" Value=' + '"' + str(data_dict["key_Duty_Average"]) + '"/>' + '\n' +
    ...
    Row_Number_Func.next_start_row_number("GRATING_EB_StartRow.txt", Next_Start_Row)

    # ----- 最終行を書き込んだファイルをGドライブに転送 -----
    #shutil.copy("GRATING_EB_StartRow.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/009_GRATING/13_ProgramUsedFile/')
