...
Operation = 'P-electrode_Deposition_Au-WetEtching'
...
Log_File = '../Log/' + Log_Folder_Name + '/023_P-electrode.log'
...
# Output_filepath = '../XML/023_P-electrode/'
...
def Main():
    ...
    Excel_File = shutil.copy(Excel_File_List[0][0], '../DataFile/023_P-electrode/')
    ...
    Start_Number = Row_Number_Func.start_row_number("P-electrode_StartRow.txt") - 500

    # ----- ExcelデータをDataFrameとして取得 -----
    Log.Log_Info(Log_File, 'Read Excel')
    df = pd.read_excel(Excel_File, header=None, sheet_name=Data_Sheet_Name, usecols="B:P", skiprows=Start_Number)

    # ----- 使わない列を落とす -----
    df = df.drop(range(5, 10), axis=1)
    ...
    while Row_Number < row_end:
        ...
        # ----- SideEtchの取得 -----
        SideEtch = []
        Col_Width = [Col_Width_1, Col_Width_2, Col_Width_3, Col_Width_4, Col_Width_5]
        for i in range(5):
            tmp = df.iloc[Row_Number, Col_Width[i]]
            if tmp is not None and type(tmp) is not str:
                SideEtch.append(tmp)
        ...
        data_dict = {
            ...
            'key_Width_Max' : max(SideEtch),
            'key_Width_Min' : min(SideEtch)
        }
    ...
    Row_Number_Func.next_start_row_number("P-electrode_StartRow.txt", Next_Start_Row)

    # ----- 最終行を書き込んだファイルをGドライブに転送 -----
    shutil.copy("P-electrode_StartRow.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/023_P-electrode/13_ProgramUsedFile/')
