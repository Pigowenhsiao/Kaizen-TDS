...
# ----- ログ書き込み先ファイルパス -----
Log_file = '../../Log/' + Log_FolderName + '/038_EA-EML_F1_Format2.log'
...
# Output_filepath = '../../XML/038_EA-EML/F1_Format2/'
...
key_type = {
    ...
    "key_Mapper_Average_Mapper_PL_AreaStandards_B2" : float,
    ...
    "AreaStandards_1" : float,
    "AreaStandards_2" : float,
    "key_Center_X" : float,
    "key_Center_Y" : float,
    ...
    "key_Deduct_Rep" : float, #2025/01/08 M27 new add
    "key_LotNumber_9": str
}


########## 対象フォルダの全ファイルを格納するリストの作成 ##########
ALL_FILES = []


########## 取得しないフォルダ名 ##########
NotUsedDir = {'2017年', '2018年', '2019年', 'ヒメジ検討', '2015', '2016', '2017', '2018', '2018', '2019','2020年'}
...
def Open_Data_Sheet(file_path):
    ...
    data_dict = {
        ...
        "AreaStandards_1" : sheet['L20'].value,
        "AreaStandards_2" : sheet['L21'].value,
        "key_Mapper_Average_Mapper_PL_AreaStandards_B2" : sheet['N6'].value,
        ...
        "key_Deduct_Rep" : sheet['M27'].value, #2025/01/08 M27 new add
...
def Output_XML(xml_file, data_dict, Data_file_sheet_array):
    ...
    XML =  '<?xml version="1.0" encoding="utf-8"?>' + '\n' + \
           ...
           '                   <Data DataType="Numeric" Name="Deduct_Rep" Units="" Value=' + '"' + str(data_dict["key_Deduct_Rep"]) + '"/>' + '\n' + \
             '               </TestStep>' + '\n' + \
            ...
            '                   <Data DataType="Numeric" Name="AreaStandards_1" Units="percent" Value=' + '"' + str(data_dict["AreaStandards_1"]) + '"/>' + '\n' + \
            '                   <Data DataType="Numeric" Name="AreaStandards_2" Units="percent" Value=' + '"' + str(data_dict["AreaStandards_2"]) + '"/>' + '\n' + \
            '                   <Data DataType="Numeric" Name="Mapper_PL_AreaStandards_B2" Units="percent" Value=' + '"' + str(data_dict["key_Mapper_Average_Mapper_PL_AreaStandards_B2"]) + '"/>' + '\n' + \
            ...
            '                   <Data DataType="Numeric" Name="Deduct_Rep" Units="" Value=' + '"' + str(data_dict["key_Deduct_Rep"]) + '"/>' + '\n' + \
            '               </TestStep>' + '\n'
# the XML TestStep19 section is 2025/01/08 M27 New add for M27
    # Cmbの付与
    ...
    XML +=  '\n' + \
            ...
            '                   <Item DeviceName="MOCVD" DeviceSerialNumber="' + "1" + '"/>' + '\n' + \
            '               </TestEquipment>' + '\n' + \
            '\n' + \
            '               <ErrorData/>' + '\n' + \
            '               <FailureData/>' + '\n' + \
...
if __name__ == '__main__':
    ...
    Path = ["Z:/PL-MAP/★PLﾏｯﾊﾟｰ判定/3ｲﾝﾁHTL13B4/EA/", "Z:/PL-MAP/★PLﾏｯﾊﾟｰ判定/3ｲﾝﾁHTL13B5/EA/","Z:/PL-MAP#3/★PLﾏｯﾊﾟｰ判定/HL13B5/F7EA/ﾛｯﾄ採番済/HL13B5-BT00（DExxx）/LD狙い波長算出済み/"] 
    # 2025/01/08 Add  "Z:/PL-MAP#3/★PLﾏｯﾊﾟｰ判定/HL13B5/F7EA/ﾛｯﾄ採番済/HL13B5-BT00（DExxx）/LD狙い波長算出済み/", "Z:/PL-MAP/★PLﾏｯﾊﾟｰ判定/3ｲﾝﾁHTL13B2/EA/"     
    for p in Path:
        ALL_FILE_FETCH(p)

    # ----- 処理を行ったファイル名をテキストファイルから呼び出す -----
    with open('EndsFile_F1_Format2.txt','r',encoding = 'utf-8') as textfile:
    ...
    EndFiles_str = "\n".join(EndFiles_list)
    with open('EndsFile_F1_Format2.txt', 'w', encoding='utf-8') as textfile:
        textfile.write(EndFiles_str)

    # ----- 最終行を書き込んだファイルをGドライブに転送 -----
    shutil.copy("EndsFile_F1_Format2.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/038_EA-EML/F1/13_ProgramUsedFile/')
