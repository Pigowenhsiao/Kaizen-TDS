...
# ----- ログ書き込み先ファイルパス -----
Log_file = '../../Log/' + Log_FolderName + '/038_EA-EML_F7_Format2.log'
...
# Output_filepath = '../../XML/038_EA-EML/F7_Format2/'
#Output_filepath = 'C:/Users/hsi67063/Downloads/TEST/'
...
key_type = {
    ...
    "key_Mapper_PL_Tails" : float, #2025/01/08 M27 new add    
    "key_LotNumber_9": str
}
...
def Open_Data_Sheet(file_path):
    ...
    data_dict = {
        ...
        "key_Mapper_Average_Mapper_PL_StrengthRatio": sheet['N4'].value ,  # modified value
        ...
        "key_Mapper_PL_Tails" : sheet['M27'].value, #2025/01/08 M27 new add        
        "key_Equipment_PLmapper": PLmapper
    }
    # ----- 作業者が0だった場合、'None'とする -----
    if data_dict['key_operator'] == 0 and isinstance(data_dict['key_operator'], int):
        data_dict["key_operator"] = "None"
    wb.close()
...
def Output_XML(xml_file, data_dict, Data_file_sheet_array):
    print(xml_file)
    print("Start_date",data_dict["key_start_date_time"])
    # ----- ログ書込：XML変換 -----
    Log.Log_Info(Log_file, 'Excel File To XML File Conversion')
    
    XML = '<?xml version="1.0" encoding="utf-8"?>' + '\n' + \
        '<Results xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">' + '\n' + \
        '       <Result startDateTime=' + '"' + data_dict["key_start_date_time"] + '"' + ' Result="Done">' + '\n' + \
        ...
        '                   <Data DataType="Numeric" Name="Mapper_PL_Tails" Units="" Value=' + '"' + str(data_dict["key_Mapper_PL_Tails"]) + '"/>' + '\n' + \
        '               </TestStep>' + '\n' + \
...
if __name__ == '__main__':
    ...
    Path = ["Z:/PL-MAP/★PLﾏｯﾊﾟｰ判定/3ｲﾝﾁHTL13B4/F7 EA/", "Z:/PL-MAP/★PLﾏｯﾊﾟｰ判定/3ｲﾝﾁHTL13B5/F7 EA/", "Z:/PL-MAP#2/★PLﾏｯﾊﾟｰ判定/HL13B4/F7EA/", "Z:/PL-MAP#2/★PLﾏｯﾊﾟｰ判定/HL13B5/F7EA/", "Z:/PL-MAP#2/★PLﾏｯﾊﾟｰ判定/HL13B5/F7LD/"]
    #Path = [ "Z:/PL-MAP#4/_PLマッパー判定/HL13B5/F7EA/ロット採番済み/"]
    for p in Path:
        ALL_FILE_FETCH(p)
    ...
    shutil.copy("EndsFile_F7_Format2.txt", 'T:/04_プロセス関係/10_共通/91_KAIZEN-TDS/01_開発/038_EA-EML/F7/13_ProgramUsedFile/')
